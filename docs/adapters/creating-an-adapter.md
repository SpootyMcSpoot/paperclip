...
Build a custom adapter to connect Staple to any agent runtime.
...
|---|---|---|
| Source | Inside `staple-fork` | Separate npm package |
| Distribution | Ships with Staple | Independent npm publish |
...
For most cases, **build an external adapter plugin**. It's cleaner, independently versioned, and doesn't require modifying Staple's source. See [External Adapters](/adapters/external-adapters) for the full guide.
...
1. Read config using safe helpers (`asString`, `asNumber`, etc.) from `@stapleai/adapter-utils/server-utils`
2. Build environment with `buildStapleEnv(agent)` plus context vars
...
|--------|--------|---------|
| `runChildProcess(cmd, opts)` | `@stapleai/adapter-utils/server-utils` | Spawn with timeout, grace, streaming |
| `buildStapleEnv(agent)` | `@stapleai/adapter-utils/server-utils` | Inject `STAPLE_*` env vars |
| `renderTemplate(tpl, data)` | `@stapleai/adapter-utils/server-utils` | `{{variable}}` substitution |
| `asString(v)` | `@stapleai/adapter-utils` | Safe config value extraction |
| `asNumber(v)` | `@stapleai/adapter-utils` | Safe number extraction |
...
For built-in adapters registered in Staple's source:
...
`format-event.ts` — pretty-prints stdout for `stapleai run --watch` using `picocolors`.
...
With these flags set, the Staple UI will automatically show the instructions bundle editor, skills management tab, and working directory field for agents using this adapter — no Staple source changes required.
...
Make Staple skills discoverable to your agent runtime without writing to the agent's working directory:
