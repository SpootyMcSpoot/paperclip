...
summary: Build, package, and distribute adapters as plugins without modifying Staple source
---

Staple supports external adapter plugins that can be installed from npm packages or local directories. External adapters work exactly like built-in adapters — they execute agents, parse output, and render transcripts — but they live in their own package and don't require changes to Staple's source code.

## Built-in vs External

| | Built-in | External |
|---|---|---|
| Source location | Inside `staple-fork/packages/adapters/` | Separate npm package or local directory |
...
| Distribution | Ships with Staple | Published to npm or linked via `file:` |
| Updates | Requires Staple release | Independent versioning |
...
```json
{
  ...
  "dependencies": {
    "@stapleai/adapter-utils": "^2026.325.0",
    "picocolors": "^1.1.0"
  },
  "devDependencies": {
    "@types/node": "^22.0.0",
...
| `staple.adapterUiParser` | Contract version for the UI parser (`"1.0.0"`) |
...
```ts
import type { ServerAdapterModule } from "@stapleai/adapter-utils";
...
export async function execute(
  ...
  // 2. Build environment with Staple vars injected
  const env = buildStapleEnv(agent);
...
#### Available Helpers from `@stapleai/adapter-utils`

| Helper | Purpose |
|--------|---------|
| `runChildProcess(command, opts)` | Spawn a child process with timeout, grace period, and streaming callbacks |
| `buildStapleEnv(agent)` | Inject `STAPLE_*` environment variables |
...
```sh
# Via the Staple UI
# Settings → Adapters → Install from npm → "my-staple-adapter"

# Or via API
curl -X POST http://localhost:3102/api/adapters \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"packageName": "my-staple-adapter"}'
```
...
Local adapters are symlinked into Staple's adapter directory. Changes to the source are picked up on server restart.

### Via adapter-plugins.json

For development, you can also edit `~/.staple/adapter-plugins.json` directly:

```json
[
  {
    "packageName": "my-staple-adapter",
...
```ts
import type { AdapterSessionCodec } from "@stapleai/adapter-utils";
...
Other Staple users can then install your adapter by package name from the UI or API.
