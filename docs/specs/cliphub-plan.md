# ClipHub: Marketplace for Staple Team Configurations

> The "app store" for whole-company AI teams — pre-built Staple configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Staple-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Staple company (agents, projects, governance) |
...
A complete Staple company configuration:
...
Single-agent configurations designed to plug into a Staple org:
...
Portable skill files that any Staple agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Staple's skill loading system
...
interface Listing {
  ...
  stapleVersionMin: string;     // Minimum Staple version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Staple company
...
| `POST` | `/api/listings/:id/install` | Install to Staple company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Staple company (or create new)
  → ClipHub API calls Staple API to:
  ...
  → Redirect to Staple dashboard with new team running
```
...
### 5.3 Creator: Export from Staple → Publish

```
Running Staple company → "Export as Blueprint" (CLI or UI)
  → Staple exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Staple brand blue/purple)
...
| Install flow | One-click deploy to Staple company |
...
{
  "targetCompanyId": "uuid",         // Existing Staple company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `staple-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Staple
- **Backend**: Node.js API (or extend Staple server)
- **Database**: Postgres (can share Staple's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Staple auth (or OAuth2)

### 9.2 Integration with Staple

ClipHub can be:
- **Option A**: A separate app that calls Staple's API to install blueprints
- **Option B**: A built-in section of the Staple UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Staple UI and API.
...
- [ ] Install flow: blueprint → Staple company
...
- [ ] "Export from Staple" CLI command
