...
Audience: Operators setting up and running agents in Staple

## 1. What this system does

Agents in Staple do not run continuously.  
...
For `claude_local` and `codex_local`, Staple assumes the CLI is already installed and authenticated on the host machine.
...
Staple stores session IDs for resumable adapters.
...
Staple pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. Staple surfaces this as a warning in environment tests, not a hard error.
