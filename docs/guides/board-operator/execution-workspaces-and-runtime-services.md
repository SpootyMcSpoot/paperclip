...
This guide documents the intended runtime model for projects, execution workspaces, and issue runs in Staple.

Staple now presents this as a workspace-command model:
...
- Staple does not automatically start or stop these workspace services as part of issue execution.
- Staple also does not automatically restart workspace services on server boot.
...
2. Staple realizes the effective execution workspace, including creating or reusing a worktree when needed.
3. Staple persists execution-workspace metadata such as paths, refs, and provisioning settings.
