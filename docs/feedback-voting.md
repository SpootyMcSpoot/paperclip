...
When you rate an agent's response with **Helpful** (thumbs up) or **Needs work** (thumbs down), Staple saves your vote locally alongside your running instance. This guide covers what gets stored, how to access it, and how to export it.
...
```bash
# Installed CLI
stapleai feedback report

# Point to a different server or company
pnpm stapleai feedback report --api-base http://127.0.0.1:3000 --company-id <company-id>

# Include raw payload dumps in the report
pnpm stapleai feedback report --payloads
```
...
feedback-export-20260331T120000Z/
  ...
  traces/
    PAP-123-e5f6g7h8.json      # Staple feedback envelope (one per trace)
  full-traces/
    PAP-123-e5f6g7h8/
...
Exports are full by default. `traces/` keeps the Staple envelope, while `full-traces/` contains the richer per-trace bundle plus any recoverable adapter-native files.

```bash
# Custom server and output directory
pnpm stapleai feedback export --api-base http://127.0.0.1:3000 --company-id <company-id> --out ./my-export
```
...
- Default endpoint: when no feedback export backend URL is configured, Staple falls back to `https://telemetry.staple.ing`
