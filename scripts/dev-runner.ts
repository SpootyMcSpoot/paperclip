...
if (worktreeEnvBootstrap.missingEnv) {
  console.error(
    `[staple] linked git worktree at ${repoRoot} is missing ${path.relative(repoRoot, worktreeEnvBootstrap.envPath)}. Run \`stapleai worktree init\` in this worktree before \`pnpm dev\`.`,
  );
  process.exit(1);
}
...
const devServerStatusFilePath = path.join(repoRoot, ".staple", "dev-server-status.json");
const devServerStatusToken = mode === "dev" ? randomUUID() : null;
const devServerStatusTokenHeader = "x-staple-dev-server-status-token";
...
for (let index = 0; index < cliArgs.length; index += 1) {
  ...
  if (arg === "--bind") {
    ...
    if (!value || value.startsWith("--") || !BIND_MODES.includes(value as BindMode)) {
      console.error(`[staple] invalid --bind value. Use one of: ${BIND_MODES.join(", ")}`);
      process.exit(1);
    }
  ...
  if (arg === "--bind-host") {
    ...
    if (!value || value.startsWith("--")) {
      console.error("[staple] --bind-host requires a value");
      process.exit(1);
    }
    bindHost = value;
    index += 1;
...
if (bindMode === "custom" && !bindHost) {
  console.error("[staple] --bind custom requires --bind-host <host>");
  process.exit(1);
}
...
if (mode === "dev") {
  env.STAPLE_DEV_SERVER_STATUS_FILE = devServerStatusFilePath;
  env.STAPLE_DEV_SERVER_STATUS_TOKEN = devServerStatusToken ?? "";
  env.STAPLE_MIGRATION_AUTO_APPLY ??= "true";
}

if (mode === "watch") {
  delete env.STAPLE_DEV_SERVER_STATUS_TOKEN;
...
if (tailscaleAuth || bindMode) {
  ...
  if (tailscaleAuth) {
    console.log("[staple] note: --tailscale-auth/--authenticated-private are legacy aliases for --bind lan");
  }
  env.STAPLE_BIND = effectiveBind;
  if (bindHost) {
    env.STAPLE_BIND_HOST = bindHost;
  } else {
    delete env.STAPLE_BIND_HOST;
  }
  if (effectiveBind === "loopback" && !tailscaleAuth) {
    delete env.STAPLE_DEPLOYMENT_MODE;
    delete env.STAPLE_DEPLOYMENT_EXPOSURE;
    delete env.STAPLE_AUTH_BASE_URL_MODE;
    console.log("[staple] dev mode: local_trusted (bind=loopback)");
  } else {
    env.STAPLE_DEPLOYMENT_MODE = "authenticated";
    env.STAPLE_DEPLOYMENT_EXPOSURE = "private";
    env.STAPLE_AUTH_BASE_URL_MODE = "auto";
    console.log(
      `[staple] dev mode: authenticated/private (bind=${effectiveBind}${bindHost ? `:${bindHost}` : ""})`,
    );
  }
} else {
  delete env.STAPLE_BIND;
  delete env.STAPLE_BIND_HOST;
  delete env.STAPLE_DEPLOYMENT_MODE;
  delete env.STAPLE_DEPLOYMENT_EXPOSURE;
  delete env.STAPLE_AUTH_BASE_URL_MODE;
...
if (existingRunner) {
  console.log(
    `[staple] ${devService.serviceName} already running (pid ${existingRunner.pid}${typeof existingRunner.metadata?.childPid === "number" ? `, child ${existingRunner.metadata.childPid}` : ""})`,
  );
  process.exit(0);
}
...
async function maybePreflightMigrations(options: { interactive?: boolean; autoApply?: boolean; exitOnDecline?: boolean } = {}) {
  ...
  const autoApply = options.autoApply ?? env.STAPLE_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[staple] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). Refusing to start watch mode against a stale schema.\n`,
      );
      process.exit(1);
    }
    return;
