#!/usr/bin/env node
/**
 * Screenshot utility for Staple UI.
 *
 * Reads the board token from ~/.staple/auth.json and injects it as a
...
const rawUrl = args[0];
const outPath = args[1] || "/tmp/staple-screenshot.png";
...
function loadBoardToken() {
  const authPath = path.resolve(os.homedir(), ".staple/auth.json");
  try {
...
if (!cred) {
  console.error("No board token found in ~/.staple/auth.json");
  process.exit(1);
}
...
(async () => {
  ...
  try {
    ...
    // Scope the auth header to the Staple origin only
