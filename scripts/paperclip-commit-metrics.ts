...
const DEFAULT_QUERY = "\"Co-Authored-By: Staple <noreply@staple.ing>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/staple-commit-metrics-cache.json");
...
const STAPLE_EMAIL = "noreply@staple.ing";
const STAPLE_NAME = "staple";
...
function printHelp() {
  console.log(`Usage: tsx scripts/staple-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === STAPLE_EMAIL) || (displayName && displayName.toLowerCase() === STAPLE_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("Staple commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "staple-commit-metrics",
