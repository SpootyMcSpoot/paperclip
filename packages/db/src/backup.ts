...
function resolveStapleHomeDir(): string {
  const envHome = process.env.STAPLE_HOME?.trim();
  if (envHome) return path.resolve(expandHomePrefix(envHome));
  return path.resolve(os.homedir(), ".staple");
}

function resolveStapleInstanceId(): string {
  const raw = process.env.STAPLE_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveStapleHomeDir(), "instances", resolveStapleInstanceId(), "config.json");
}
...
function resolveDefaultBackupDir(): string {
  return path.resolve(resolveStapleHomeDir(), "instances", resolveStapleInstanceId(), "data", "backups");
}
