...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://staple:staple@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "staple");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://staple:staple@127.0.0.1:${preferredPort}/staple`,
  ...
  if (runningPid) {
    ...
    const adminConnectionString = `postgres://staple:staple@127.0.0.1:${port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "staple");
    return {
      connectionString: `postgres://staple:staple@127.0.0.1:${port}/staple`,
  ...
  const adminConnectionString = `postgres://staple:staple@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "staple");

  return {
    connectionString: `postgres://staple:staple@127.0.0.1:${selectedPort}/staple`,
