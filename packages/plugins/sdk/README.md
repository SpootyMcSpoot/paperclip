...
Official TypeScript SDK for Staple plugin authors.

- **Worker SDK:** `@stapleai/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@stapleai/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@stapleai/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@stapleai/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@stapleai/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@stapleai/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@stapleai/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@stapleai/plugin-sdk/ui/hooks` | Hooks only |
| `@stapleai/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@stapleai/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@stapleai/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@stapleai/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@stapleai/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@stapleai/plugin-sdk/types` | Worker context and API types (advanced) |
...
- Plugin UI bundles run as same-origin JavaScript inside the main Staple app. They can call ordinary Staple HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Staple instance at runtime.
...
```ts
import { definePlugin, runWorker } from "@stapleai/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@stapleai/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core Staple information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@stapleai/plugin-sdk`.
...
const child = await ctx.issues.create({
  ...
  originKind: "plugin:staple.missions:feature",
...
```tsx
import { usePluginData, usePluginAction } from "@stapleai/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@stapleai/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@stapleai/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@stapleai/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@stapleai/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@stapleai/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@stapleai/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@stapleai/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@stapleai/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@stapleai/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@stapleai/plugin-sdk/bundlers";
...
```bash
staple-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@stapleai/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__staple__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__staple__/events` streams `reload` SSE events on UI build changes
