/**
 * Core types for the Staple plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@stapleai/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @stapleai/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of Staple object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the Staple UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit Staple domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Staple domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the Staple secret provider
...
export interface PluginSecretsClient {
   ...
   * in the Staple secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@stapleai/plugin-sdk/ui`
 */
 ...
 * @example
 * ```ts
 * import { definePlugin } from "@stapleai/plugin-sdk";
 *
