 ...
 * channel.
 *
 * The Staple plugin runtime uses JSON-RPC 2.0 over stdio to communicate
...
export type { PluginLauncherRenderContextSnapshot } from "@stapleai/shared";
 ...
 * we use strings (UUIDs or monotonic counters) for all Staple messages.
 */
...
/**
 * Staple plugin-specific error codes.
 *
...
export interface InitializeParams {
  ...
  instanceInfo: {
    /** UUID of this Staple instance. */
    instanceId: string;
    /** Semver version of the running Staple host. */
    hostVersion: string;
  };
  /** Host API version. */
  apiVersion: number;
