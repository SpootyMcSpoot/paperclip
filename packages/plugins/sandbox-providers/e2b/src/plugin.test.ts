...
describe("E2B sandbox provider plugin", () => {
  ...
  it("uses resolved config keys before falling back to E2B_API_KEY", async () => {
    ...
    expect(lease).toMatchObject({
      ...
      metadata: {
        provider: "e2b",
        remoteCwd: "/home/user/staple-workspace",
      },
    });
    expect(sandbox.commands.run).toHaveBeenNthCalledWith(1, "pwd");
    expect(sandbox.commands.run).toHaveBeenNthCalledWith(2, "mkdir -p '/home/user/staple-workspace'");
  });
  ...
  it("creates the remote workspace before returning it", async () => {
    ...
    await expect(plugin.definition.onEnvironmentRealizeWorkspace?.({
      ...
      lease: {
        ...
        metadata: { remoteCwd: "/home/user/staple-workspace" },
      },
      workspace: {
        localPath: "/tmp/staple-workspace",
      },
    })).resolves.toEqual({
      cwd: "/home/user/staple-workspace",
      metadata: {
        provider: "e2b",
        remoteCwd: "/home/user/staple-workspace",
      },
    ...
    expect(sandbox.commands.run).toHaveBeenCalledWith("mkdir -p '/home/user/staple-workspace'");
  });
