...
describe("ssh env-lab fixture", () => {
  ...
  it("starts an isolated sshd fixture and executes commands through it", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "staple-ssh-fixture-"));
    ...
    await expect(
      buildSshSpawnTarget({
        spec: {
          ...
          port: 22,
          username: "ssh-user",
          remoteCwd: "/srv/staple/workspace",
  ...
  it("syncs a local directory into the remote fixture workspace", async () => {
    ...
    await writeFile(path.join(localDir, "message.txt"), "hello from staple\n", "utf8");
    ...
    expect(result.stdout).toContain("hello from staple");
  ...
  it("round-trips a git workspace through the SSH fixture", async () => {
    ...
    await git(localRepo, ["config", "user.name", "Staple Test"]);
    await git(localRepo, ["config", "user.email", "test@staple.dev"]);
    ...
    await runSshCommand(
      config,
      `sh -lc 'cd ${JSON.stringify(started.workspaceDir)} && git config user.name "Staple SSH" && git config user.email "ssh@staple.dev" && git add tracked.txt untracked.txt && git commit -m "remote update" >/dev/null && printf "remote dirty\\n" > tracked.txt && printf "remote extra\\n" > remote-only.txt'`,
