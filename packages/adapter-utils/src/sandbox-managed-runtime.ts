...
export function parseSandboxRemoteExecutionSpec(value: unknown): SandboxRemoteExecutionSpec | null {
  ...
  return {
    ...
    stapleApiUrl: asString(parsed.stapleApiUrl).trim() || null,
  };
}
...
export function buildSandboxExecutionSessionIdentity(spec: SandboxRemoteExecutionSpec | null) {
  ...
  return {
    ...
    ...(spec.stapleApiUrl ? { stapleApiUrl: spec.stapleApiUrl } : {}),
  } as const;
}
...
export function sandboxExecutionSessionMatches(saved: unknown, current: SandboxRemoteExecutionSpec | null): boolean {
  ...
  return (
    ...
    asString(parsedSaved.stapleApiUrl) === asString(currentIdentity.stapleApiUrl)
  );
}
...
export async function prepareSandboxManagedRuntime(input: {
  ...
  const runtimeRootDir = path.posix.join(workspaceRemoteDir, ".staple-runtime", input.adapterKey);

  await withTempDir("staple-sandbox-sync-", async (tempDir) => {
    ...
    const preservedNames = new Set([".staple-runtime", ...(input.preserveAbsentOnRestore ?? [])]);
  ...
  return {
    ...
    restoreWorkspace: async () => {
      await withTempDir("staple-sandbox-restore-", async (tempDir) => {
        ...
        await mirrorDirectory(extractedDir, input.workspaceLocalDir, {
          preserveAbsent: [".staple-runtime", ...(input.preserveAbsentOnRestore ?? [])],
        });
      });
    },
  };
