...
export function parseSshRemoteExecutionSpec(value: unknown): SshRemoteExecutionSpec | null {
  ...
  return {
    ...
    remoteCwd,
    stapleApiUrl:
      typeof parsed.stapleApiUrl === "string" && parsed.stapleApiUrl.trim().length > 0
        ? parsed.stapleApiUrl.trim()
        : null,
    remoteWorkspacePath:
...
export async function findReachableStapleApiUrlOverSsh(input: {
...
async function createSshAuthArgs(
  ...
  if (config.strictHostKeyChecking) {
    if (config.knownHosts) {
      const knownHosts = await withTempFile("staple-ssh-known-hosts-", config.knownHosts, 0o600);
  ...
  if (config.privateKey) {
    const privateKey = await withTempFile("staple-ssh-key-", config.privateKey, 0o600);
...
async function importGitWorkspaceToSsh(input: {
  ...
  const bundleDir = await fs.mkdtemp(path.join(os.tmpdir(), "staple-ssh-bundle-"));
  const bundlePath = path.join(bundleDir, "workspace.bundle");
  const tempRef = "refs/staple/ssh-sync/import";
  ...
  try {
    ...
    const remoteSetupScript = [
      "set -e",
      `mkdir -p ${shellQuote(path.posix.join(input.remoteDir, ".staple-runtime"))}`,
      `tmp_bundle=$(mktemp ${shellQuote(path.posix.join(input.remoteDir, ".staple-runtime", "import-XXXXXX.bundle"))})`,
      ...
      `git -C ${shellQuote(input.remoteDir)} clean -fdx -e .staple-runtime >/dev/null`,
    ].join("\n");
...
async function exportGitWorkspaceFromSsh(input: {
  ...
  const bundleDir = await fs.mkdtemp(path.join(os.tmpdir(), "staple-ssh-bundle-"));
  const bundlePath = path.join(bundleDir, "workspace.bundle");
  const importedRef = "refs/staple/ssh-sync/imported";

  try {
    const exportScript = [
      "set -e",
      `git -C ${shellQuote(input.remoteDir)} update-ref refs/staple/ssh-sync/export HEAD`,
      `mkdir -p ${shellQuote(path.posix.join(input.remoteDir, ".staple-runtime"))}`,
      `tmp_bundle=$(mktemp ${shellQuote(path.posix.join(input.remoteDir, ".staple-runtime", "export-XXXXXX.bundle"))})`,
      'cleanup() { rm -f "$tmp_bundle"; git -C ' + shellQuote(input.remoteDir) + ' update-ref -d refs/staple/ssh-sync/export >/dev/null 2>&1 || true; }',
      'trap cleanup EXIT',
      `git -C ${shellQuote(input.remoteDir)} bundle create "$tmp_bundle" refs/staple/ssh-sync/export >/dev/null`,
      'cat "$tmp_bundle"',
    ].join("\n");
    ...
    await runLocalGit(input.localDir, ["fetch", "--force", bundlePath, `refs/staple/ssh-sync/export:${importedRef}`], {
      timeout: 60_000,
      maxBuffer: 1024 * 1024,
    });
...
export async function syncDirectoryFromSsh(input: {
  ...
  const auth = await createSshAuthArgs(input.spec);
  const stagingDir = await fs.mkdtemp(path.join(os.tmpdir(), "staple-ssh-sync-back-"));
...
export async function prepareWorkspaceForSshExecution(input: {
  ...
  await clearRemoteDirectory({
    spec: input.spec,
    remoteDir,
    preserveEntries: [".staple-runtime"],
  });
  await syncDirectoryToSsh({
    spec: input.spec,
    localDir: input.localDir,
    remoteDir,
    exclude: [".staple-runtime"],
  });
}
