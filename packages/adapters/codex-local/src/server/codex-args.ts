import { asBoolean, asString, asStringArray } from "@stapleai/adapter-utils/server-utils";
import {
  ...
  return {
    ...
    fastModeIgnoredReason:
      fastModeRequested && !fastModeApplied
        ? `Configured fast mode is currently only supported on ${formatFastModeSupportedModels()}; Staple will ignore it for model ${model || "(default)"}.`
        : null,
  };
}
