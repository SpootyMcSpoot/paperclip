...
import type { AdapterModel } from "@stapleai/adapter-utils";
import { asString, runChildProcess } from "@stapleai/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.STAPLE_PI_COMMAND === "string" &&
    process.env.STAPLE_PI_COMMAND.trim().length > 0
      ? process.env.STAPLE_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["STAPLE_", "npm_", "NPM_"] as const;
