...
## Scope
This plan is now **gateway-only**. Staple supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Staple skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a Staple task.
...
```bash
pnpm --filter @stapleai/server test:run -- openclaw-gateway
