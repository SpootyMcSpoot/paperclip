...
const DEFAULT_CLIENT_VERSION = "staple";
...
function resolveStapleApiUrlOverride(value: unknown): string | null {
...
function buildStapleEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  ...
  const stapleEnv: Record<string, string> = {
    ...buildStapleEnv(ctx.agent),
    STAPLE_RUN_ID: ctx.runId,
  };

  if (stapleApiUrlOverride) {
    stapleEnv.STAPLE_API_URL = stapleApiUrlOverride;
  }
  if (wakePayload.taskId) stapleEnv.STAPLE_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) stapleEnv.STAPLE_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) stapleEnv.STAPLE_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) stapleEnv.STAPLE_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) stapleEnv.STAPLE_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    stapleEnv.STAPLE_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }
...
function buildWakeText(
  ...
  const orderedKeys = [
    ...
    "STAPLE_TASK_ID",
    "STAPLE_WAKE_REASON",
    "STAPLE_WAKE_COMMENT_ID",
    "STAPLE_APPROVAL_ID",
    "STAPLE_APPROVAL_STATUS",
    "STAPLE_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = stapleEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = stapleEnv.STAPLE_API_URL ?? "<set STAPLE_API_URL>";

  const lines = [
    "Staple wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `STAPLE_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load STAPLE_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $STAPLE_API_KEY on every API call.",
    "- Use X-Staple-Run-Id: $STAPLE_RUN_ID on every mutating API call.",
...
function buildStandardStaplePayload(
  ...
  const templateStaple = parseObject(payloadTemplate.staple);
  const workspace = asRecord(ctx.context.stapleWorkspace);
  const workspaces = Array.isArray(ctx.context.stapleWorkspaces)
    ? ctx.context.stapleWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.stapleRuntimeServiceIntents)
    ? ctx.context.stapleRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardStaple: Record<string, unknown> = {
    runId: ctx.runId,
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "staple-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const staplePayload = buildStandardStaplePayload(ctx, wakePayload, stapleEnv, payloadTemplate);
