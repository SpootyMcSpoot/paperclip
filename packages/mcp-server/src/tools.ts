...
async function getIssueWorkspaceRuntime(client: StapleApiClient, issueId: string) {
...
export function createToolDefinitions(client: StapleApiClient): ToolDefinition[] {
  return [
    makeTool(
      "stapleMe",
      "Get the current authenticated Staple actor details",
      z.object({}),
    ...
    makeTool(
      "stapleApiRequest",
      "Make a JSON request to an existing Staple /api endpoint for unsupported operations",
      apiRequestSchema,
