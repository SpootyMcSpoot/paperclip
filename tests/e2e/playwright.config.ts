...
const PORT = Number(process.env.STAPLE_E2E_PORT ?? 3199);
const BASE_URL = `http://127.0.0.1:${PORT}`;
const STAPLE_HOME = fs.mkdtempSync(path.join(os.tmpdir(), "staple-e2e-home-"));
...
export default defineConfig({
  ...
  // `onboard --yes --run` works in a non-interactive temp STAPLE_HOME.
  webServer: {
    command: `pnpm stapleai onboard --yes --run`,
    url: `${BASE_URL}/api/health`,
    // Always boot a dedicated throwaway instance for e2e so browser tests
    // never attach to the developer's active Staple home/server.
    ...
    env: {
      ...
      PORT: String(PORT),
      STAPLE_HOME,
      STAPLE_INSTANCE_ID: "playwright-e2e",
      STAPLE_BIND: "loopback",
      STAPLE_DEPLOYMENT_MODE: "local_trusted",
