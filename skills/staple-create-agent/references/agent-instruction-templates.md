...
These templates are deliberately separate from the main Staple heartbeat skill and from `SKILL.md` in this folder — the core wake procedure and hiring workflow stay short, and role-specific depth lives here.
...
2. Copy that template into the new agent's instruction bundle (usually `AGENTS.md`). For hire requests using local managed-bundle adapters, set the adapted template as `adapterConfig.promptTemplate`; Staple materializes it into `AGENTS.md`.
...
5. Keep the Staple heartbeat requirement and the task-comment requirement.
