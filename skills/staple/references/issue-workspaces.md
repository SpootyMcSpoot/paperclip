...
```sh
curl -sS -H "Authorization: Bearer $STAPLE_API_KEY" \
  "$STAPLE_API_URL/api/issues/$STAPLE_TASK_ID/heartbeat-context"
```
...
If `currentExecutionWorkspace` is `null`, the issue does not currently have a realized execution workspace. For child/follow-up work, create the child with `parentId` or use `inheritExecutionWorkspaceFromIssueId` so Staple preserves workspace continuity.

## Control Services

Prefer Staple-managed runtime service controls over manual `pnpm dev &` or ad-hoc background processes. These endpoints keep service state, URLs, logs, and ownership visible to other agents and the board.

```sh
# Start all configured services; waits for configured readiness checks.
curl -sS -X POST \
  -H "Authorization: Bearer $STAPLE_API_KEY" \
  -H "X-Staple-Run-Id: $STAPLE_RUN_ID" \
  -H "Content-Type: application/json" \
  "$STAPLE_API_URL/api/execution-workspaces/<workspace-id>/runtime-services/start" \
  -d '{}'
...
curl -sS -X POST \
  ...
  -H "X-Staple-Run-Id: $STAPLE_RUN_ID" \
  -H "Content-Type: application/json" \
  "$STAPLE_API_URL/api/execution-workspaces/<workspace-id>/runtime-services/restart" \
  -d '{}'
...
curl -sS -X POST \
  ...
  -H "X-Staple-Run-Id: $STAPLE_RUN_ID" \
  -H "Content-Type: application/json" \
  "$STAPLE_API_URL/api/execution-workspaces/<workspace-id>/runtime-services/stop" \
  -d '{}'
```
...
When the Staple MCP tools are available, prefer these issue-scoped tools:

- `stapleGetIssueWorkspaceRuntime` — reads `currentExecutionWorkspace` and service URLs for an issue.
- `stapleControlIssueWorkspaceServices` — starts, stops, or restarts the current issue workspace services.
- `stapleWaitForIssueWorkspaceService` — waits until a selected service is running and returns its URL when exposed.
