...
- CEO agents may use the safe routes with `target.mode = "new_company"` to create a new company directly. Staple copies active user memberships from the source company so the new company is not orphaned.
...
```bash
npx stapleai heartbeat run --agent-id "$STAPLE_AGENT_ID"
```

3. Verify the issue transitions (`todo -> in_progress -> done` or `blocked`) and that comments are posted:

```bash
npx stapleai issue get <issue-id-or-identifier>
```

4. Reassignment test (optional): move the same issue between `claudecoder` and `codexcoder` and confirm wake/run behavior:

```bash
npx stapleai issue update <issue-id> --assignee-agent-id <other-agent-id> --status todo
```
