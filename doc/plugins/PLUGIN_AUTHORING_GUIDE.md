...
This guide describes the current, implemented way to create a Staple plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Staple app.
...
```bash
pnpm --filter @stapleai/create-staple-plugin build
node packages/plugins/create-staple-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the Staple repo:

```bash
pnpm --filter @stapleai/create-staple-plugin build
node packages/plugins/create-staple-plugin/dist/index.js @yourscope/plugin-name \
...
Inside this monorepo, the scaffold uses `workspace:*` for `@stapleai/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@stapleai/plugin-sdk` from the local Staple checkout into a `.staple-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into Staple from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@stapleai/plugin-sdk/ui`
