# Staple Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Staple's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Staple app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Staple HTTP APIs directly.
...
Staple plugin design is based on the following assumptions:

1. Staple is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Staple business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Staple core.
...
2. Let plugins add major capabilities without editing Staple core.
...
4. Depend on project-local plugin folders such as `.staple/plugins`.
...
The single Staple deployment an operator installs and controls.

### 5.2 Company

A first-class Staple business object inside the instance.
...
A trusted in-process extension loaded directly by Staple core.
...
An installable instance-wide extension package loaded through the Staple plugin runtime.
...
Staple has two extension classes.
...
Staple already has a concrete workspace model:
...
Plugins live under the Staple instance directory.

Suggested layout:

- `~/.staple/instances/default/plugins/package.json`
- `~/.staple/instances/default/plugins/node_modules/`
- `~/.staple/instances/default/plugins/.cache/`
- `~/.staple/instances/default/data/plugins/<plugin-id>/`
...
Staple should add CLI commands:

- `pnpm stapleai plugin list`
- `pnpm stapleai plugin install <package[@version]>`
- `pnpm stapleai plugin uninstall <plugin-id>`
- `pnpm stapleai plugin upgrade <plugin-id> [version]`
- `pnpm stapleai plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@staple/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface StaplePluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that Staple agents can use during runs.
...
- Staple server starts one worker process per installed plugin
...
Receives one typed Staple domain event.
...
export function definePlugin(definition: PluginDefinition): StaplePlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: StaplePluginManifestV1;
  config: {
...
```ts
ctx.events.on("plugin.@staple/plugin-git.push-detected", async (event) => {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@stapleai/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@stapleai/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@stapleai/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@stapleai/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@stapleai/plugin-sdk/ui`
...
The `@stapleai/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Staple secret provider system rather than a plain text input
...
1. Core Staple data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Staple objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Staple product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Staple later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Staple secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm stapleai plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Staple server. This is a normative requirement, not optional.
...
### 27.1 `@stapleai/plugin-test-harness`
...
```ts
import { createTestHarness } from "@stapleai/plugin-test-harness";
...
For developing a plugin against a running Staple instance:

- The operator installs the plugin from a local path: `pnpm stapleai plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-staple-plugin`) that scaffolds:

- `package.json` with correct `staplePlugin` keys
...
- `@staple/plugin-workspace-files`
- `@staple/plugin-terminal`
- `@staple/plugin-git`
- `@staple/plugin-linear`
- `@staple/plugin-github-issues`
- `@staple/plugin-grafana`
- `@staple/plugin-runtime-processes`
- `@staple/plugin-stripe`
...
- `@stapleai/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@stapleai/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@stapleai/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@stapleai/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@stapleai/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@stapleai/plugin-sdk/ui`
...
- `@stapleai/plugin-test-harness`
- `create-staple-plugin` starter template
...
Staple should not implement a generic in-process hook bag modeled directly after local coding tools.

Staple should implement:
...
That is the complete target design for the Staple plugin system.
