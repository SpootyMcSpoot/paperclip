...
Staple uses PostgreSQL via [Drizzle ORM](https://orm.drizzle.team/). There are three ways to run the database, from simplest to most production-ready.
...
1. Creates a `~/.staple/instances/default/db/` directory for storage
2. Ensures the `staple` database exists
3. Runs migrations automatically for empty databases
4. Starts serving requests

Data persists across restarts in `~/.staple/instances/default/db/`. To reset local dev data, delete that directory.
...
When `DATABASE_URL` is unset, this command targets the current embedded PostgreSQL instance for your active Staple config/instance.
...
Docker note: the Docker quickstart image also uses embedded PostgreSQL by default. Persist `/staple` to keep DB state across container restarts (see `doc/DOCKER.md`).
...
# DATABASE_URL=postgres://staple:staple@localhost:5432/staple
```
...
`DATABASE_MIGRATION_URL` to the direct connection URL. Staple uses it for
...
| `DATABASE_URL` | Mode |
|---|---|
| Not set | Embedded PostgreSQL (`~/.staple/instances/default/db/`) |
...
Staple stores secret metadata and versions in:
...
- Default key file: `~/.staple/instances/default/secrets/master.key` (auto-created if missing).
- CLI config location: `~/.staple/instances/default/config.json` under `secrets.localEncrypted.keyFilePath`.

Optional overrides:

- `STAPLE_SECRETS_MASTER_KEY` (32-byte key as base64, hex, or raw 32-char string)
- `STAPLE_SECRETS_MASTER_KEY_FILE` (custom key file path)
