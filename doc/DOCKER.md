...
Staple server + PostgreSQL 17. The database is health-checked before the server starts.
...
PostgreSQL data persists in a named Docker volume (`pgdata`). Staple data persists in `staple-data`.
...
The `docker/quadlet/` directory contains unit files to run Staple + PostgreSQL as systemd services via Podman Quadlet.

| File | Purpose |
|------|---------|
| `docker/quadlet/staple.pod` | Pod definition — groups containers into a shared network namespace |
| `docker/quadlet/staple.container` | Staple server — joins the pod, connects to Postgres at `127.0.0.1` |
| `docker/quadlet/staple-db.container` | PostgreSQL 17 — joins the pod, health-checked |
...
3. Create a secrets env file (keep out of version control):
   ...
   ```sh
   cat > ~/.config/containers/systemd/staple.env <<EOL
   BETTER_AUTH_SECRET=$(openssl rand -hex 32)
   POSTGRES_USER=staple
   POSTGRES_PASSWORD=staple
   POSTGRES_DB=staple
   DATABASE_URL=postgres://staple:staple@127.0.0.1:5432/staple
   # OPENAI_API_KEY=sk-...
...
4. Create the data directory and start:
   ...
   ```sh
   mkdir -p ~/.local/share/staple
   systemctl --user daemon-reload
   systemctl --user start staple-pod
   ```

### Quadlet management

```sh
journalctl --user -u staple -f        # App logs
journalctl --user -u staple-db -f     # DB logs
systemctl --user status staple-pod    # Pod status
systemctl --user restart staple-pod   # Restart all
systemctl --user stop staple-pod      # Stop all
```

### Quadlet notes

- **First boot**: Unlike Docker Compose's `condition: service_healthy`, Quadlet's `After=` only waits for the DB unit to *start*, not for PostgreSQL to be ready. On a cold first boot you may see one or two restart attempts in `journalctl --user -u staple` while PostgreSQL initialises — this is expected and resolves automatically via `Restart=on-failure`.
- Containers in a pod share `localhost`, so Staple reaches Postgres at `127.0.0.1:5432`.
- PostgreSQL data persists in the `staple-pgdata` named volume.
- Staple data persists at `~/.local/share/staple`.
...
- Staple data persists via Docker volumes/bind mounts (compose) or at `~/.local/share/staple` (quadlet).
