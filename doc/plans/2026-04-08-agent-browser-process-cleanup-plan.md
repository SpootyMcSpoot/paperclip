...
Yes, there is a likely root cause in Staple's local execution model.
...
If Codex, Claude, Cursor, or a skill launched through them starts Chrome or Chromium helpers, Staple can lose ownership of those descendants even when it still believes it handled the run correctly.
...
This is the clearest internal comparison point: Staple already has one local-process subsystem that treats process-group ownership as the right abstraction.
...
- Staple may think it cancelled the run because the parent process is gone
...
This work is successful when Staple can:
...
- require every skill author to add bespoke browser teardown logic before Staple can clean up correctly
...
- make the leak measurable from Staple's side before changing execution semantics
...
- add a control-plane cleanup action or CLI utility for stale local run processes owned by Staple
...
Treat this as a Staple executor ownership bug, not an `agent-browser` bug.
