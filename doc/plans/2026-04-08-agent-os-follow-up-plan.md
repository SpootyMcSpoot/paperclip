...
Turn the `agent-os` research into a low-risk Staple execution plan that preserves Staple's control-plane model while testing the few runtime ideas that appear worth adopting.

## Decision summary

Staple should not absorb `agent-os` as a product model or orchestration layer.

Staple should evaluate `agent-os` in three narrow areas:
...
This work is successful when Staple has:
...
- a proposed runtime capability model that fits current Staple adapters
...
- replace Staple heartbeats, issues, comments, approvals, or budgets with `agent-os` primitives
- introduce Rust/sidecar requirements for all local execution paths
- migrate all adapters at once
- add runtime workflow/queue abstractions to Staple core

## Existing Staple integration points
...
- prove that Staple can drive one local agent through an `agent-os` runtime without breaking heartbeat semantics
...
- keep Staple's existing heartbeat, issue, workspace, and comment flow authoritative
...
- introduce a Staple-native capability vocabulary without coupling the product to `agent-os`
...
- determine whether a layered/snapshotted root model improves some Staple workloads
...
- identify where Staple should prefer explicit typed tools over ambient shell access
...
- one concrete proposal for typed-tool adoption in Staple
...
- runtime experimentation could blur the boundary between Staple as control plane and Staple as execution platform
