# Staple Module System

## Overview

Staple's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Staple's API, UI, and data model | Yes |
...
modules/
  observability/
    staple.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@stapleai/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@stapleai/core";
...
```
1. Scan modules/ directory for staple.module.json manifests
...
```jsonc
// staple.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@stapleai/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@stapleai/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:staple/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.staple.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm stapleai store list                    # browse available modules and templates
pnpm stapleai store install <module-id>     # install a module
pnpm stapleai store import <template-id>    # import a company template
pnpm stapleai store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Staple instance | `server:started` |
...
4. **Module config** — `staple.config.json` with per-module config, env var interpolation
...
10. **`@stapleai/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
