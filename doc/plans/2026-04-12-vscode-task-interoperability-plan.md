...
Staple should not replace its workspace runtime service model with VS Code tasks.
...
- Staple owns long-running workspace services and their desired state
- Staple shows operators exactly which named thing they are starting or stopping
- Staple distinguishes long-running services from one-shot jobs
...
This is directionally correct for Staple because it gives the control plane an explicit model for service lifecycle, health, reuse, and restart behavior.
...
The issue is that the configuration surface is too low-level and Staple does not yet leverage existing workspace metadata.

## Recommendation

Keep Staple runtime services as the source of truth for service supervision.
...
4. `Runtime service instance`
   The live process record that already exists today in Staple.

In that model, VS Code tasks are a way to populate workspace commands.
Only commands that map cleanly to Staple service or job semantics should become runnable in Staple.

## Why Not Fully Adopt VS Code Tasks

VS Code tasks are broader than Staple runtime services.
...
That creates a bad fit if Staple tries to use `tasks.json` as its only runtime model:
...
- problem matcher readiness is useful metadata, but it is not enough to replace Staple's persisted service lifecycle model

The right boundary is interoperability, not replacement.

## Interoperability Contract

Staple should support a conservative subset of VS Code tasks and clearly mark unsupported entries.

### Supported in phase 1

- `shell` and `process` tasks with a concrete command Staple can resolve
...
- selected variable substitution when Staple can resolve it safely from workspace context
- mapping task metadata into Staple readiness/expose hints
- limited compound-task launch flows

### Not supported initially

- extension-provided task types Staple cannot execute directly
...
Staple needs an explicit classification layer instead of assuming every VS Code task is a service.

Recommended classification:

- `service`
  Explicitly marked by Staple metadata, or confidently inferred from background/watch task semantics

- `job`
  One-shot command expected to exit

- `unsupported`
  Present in `tasks.json`, but not safely runnable by Staple
...
For a workspace with `cwd`, Staple should look for `.vscode/tasks.json`.
...
Staple should map imported tasks with explicit, documented rules.

Recommended rules:

1. A task becomes a `job` by default.
2. A task becomes a `service` only when:
   - Staple metadata marks it as a service, or
...
Staple-specific metadata can live in a namespaced field on the imported task definition, for example:
...
Success condition:
an operator can see what Staple would import and why.
...
Goal:
run supported imported service commands through the current Staple supervisor.
...
Success condition:
imported service commands behave like native Staple services once adopted.
...
- removal of Staple's own runtime supervision model
- editor-dependent execution semantics inside the control plane

## Risks

- overfitting Staple to VS Code and making the model worse for non-VS-Code repos
...
Staple should adopt VS Code tasks as an optional workspace command source, not as the canonical runtime model.
...
- Existing Staple workspace plan: `doc/plans/2026-03-10-workspace-strategy-and-git-worktrees.md`
