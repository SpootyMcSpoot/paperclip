...
This spec defines how Staple actually runs agents while staying runtime-agnostic.
...
1. Staple is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Staple manages lifecycle, and on exit Staple parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `staple.skill` (shared Staple skill text block)
...
2. Preferred transport is env vars (`STAPLE_*`) injected at runtime.
