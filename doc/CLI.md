...
- `stapleai run --bind <loopback|lan|tailnet>` passes a quickstart bind preset into first-run onboarding when config is missing
- runtime can override mode with `STAPLE_DEPLOYMENT_MODE`
- `stapleai run` and `stapleai doctor` still do not expose a direct low-level `--mode` flag
...
```sh
pnpm stapleai env-lab up
pnpm stapleai env-lab doctor
pnpm stapleai env-lab status --json
pnpm stapleai env-lab down
```
