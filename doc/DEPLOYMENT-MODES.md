...
Staple supports two runtime modes:
...
Staple now treats **bind** as a separate concern from auth:
...
pnpm stapleai onboard --yes --bind lan
pnpm stapleai run --bind tailnet
```
...
When running `authenticated` mode, if the only instance admin is `local-board`, Staple emits a startup warning with a one-time high-entropy claim URL.
