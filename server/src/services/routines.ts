...
import { trackRoutineRun } from "@stapleai/shared/telemetry";
...
function resolveRoutineVariableValues(
  ...
  for (const variable of variables) {
    ...
    // Staple manages from execution context, so callers cannot override them.
  ...
  return {
    ...
    createTrigger: async (
      ...
      if (input.kind === "webhook") {
        ...
        secretMaterial = {
          webhookUrl: `${process.env.STAPLE_API_URL}/api/routine-triggers/public/${publicId}/fire`,
    ...
    rotateTriggerSecret: async (
      ...
      return {
        ...
        secretMaterial: {
          webhookUrl: `${process.env.STAPLE_API_URL}/api/routine-triggers/public/${existing.publicId}/fire`,
    ...
    firePublicTrigger: async (publicId: string, input: {
      ...
      if (trigger.signingMode === "none") {
        ...
        // generic X-Staple-Signature header so operators can use github_hmac
