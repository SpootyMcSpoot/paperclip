 ...
 *    (`~/.staple/plugins/`) and `node_modules` for packages matching
 *    the `staple-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published Staple plugins.
 * Packages matching this pattern are considered Staple plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "staple-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: StaplePluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.staple/plugins/ local directory
  | "npm"               // npm packages matching staple-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: StaplePluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the staple-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "staple-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: StaplePluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover Staple plugins installed as npm packages in the current
   * Node.js environment matching the "staple-plugin-*" naming convention.
   *
   ...
   * the "staplePlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a Staple plugin.
   * Throws if the package is a Staple plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<StaplePluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: StaplePluginManifestV1;
    newManifest: StaplePluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the Staple plugin naming convention.
 * Accepts both the "staple-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @stapleai/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "staplePlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const staplePlugin = pkgJson["staplePlugin"];
  if (
    staplePlugin !== null &&
    typeof staplePlugin === "object" &&
    !Array.isArray(staplePlugin)
  ) {
    const manifestRelPath = (staplePlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: StaplePluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumStapleVersion;
}
 ...
 *   packageName: "staple-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: StaplePluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as StaplePluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Staple plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<StaplePluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a Staple plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasStaplePlugin = "staplePlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasStaplePlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<StaplePluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: StaplePluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @stapleai/shared exports). Run those workers through
