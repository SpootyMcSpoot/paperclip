...
import type { Environment, EnvironmentLease } from "@stapleai/shared";
import {
...
export async function resolveEnvironmentExecutionTarget(input: {
  ...
  if (input.environment.driver === "sandbox") {
    ...
    const stapleApiUrl =
      typeof input.leaseMetadata?.stapleApiUrl === "string" && input.leaseMetadata.stapleApiUrl.trim().length > 0
        ? input.leaseMetadata.stapleApiUrl.trim()
        : typeof process.env.STAPLE_RUNTIME_API_URL === "string" && process.env.STAPLE_RUNTIME_API_URL.trim().length > 0
          ? process.env.STAPLE_RUNTIME_API_URL.trim()
          : null;
