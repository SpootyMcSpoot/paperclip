...
import { companySkills } from "@stapleai/db";
import { readStapleSkillSyncPreference } from "@stapleai/adapter-utils/server-utils";
import type { StapleSkillEntry } from "@stapleai/adapter-utils/server-utils";
import type {
  CompanySkill,
...
import { normalizeAgentUrlKey } from "@stapleai/shared";
import { resolveStapleInstanceRoot } from "../home-paths.js";
...
function buildSkillRuntimeName(key: string, slug: string) {
  if (key.startsWith("stapleai/staple/")) return slug;
...
function resolveManagedSkillsRoot(companyId: string) {
  return path.resolve(resolveStapleInstanceRoot(), "skills", companyId);
}
...
function deriveSkillSourceInfo(skill: SkillSourceInfoTarget): {
  ...
  if (metadata.sourceKind === "staple_bundled") {
    return {
      editable: false,
      editableReason: "Bundled Staple skills are read-only.",
      sourceLabel: "Staple bundled",
      sourceBadge: "staple",
      sourcePath: null,
    };
  }
...
export function companySkillService(db: Db) {
  ...
  async function listRuntimeSkillEntries(
    ...
    for (const skill of skills) {
      ...
      const required = sourceKind === "staple_bundled";
      out.push({
        key: skill.key,
        ...
        requiredReason: required
          ? "Bundled Staple skills are always available for local adapters."
          : null,
      });
    }
  ...
  async function upsertImportedSkills(companyId: string, imported: ImportedSkill[]): Promise<CompanySkill[]> {
    ...
    for (const skill of imported) {
      ...
      if (
        existing
        && existingMeta.sourceKind === "staple_bundled"
        && incomingKind === "github"
        && incomingOwner === "stapleai"
        && incomingRepo === "staple"
      ) {
        out.push(existing);
        continue;
