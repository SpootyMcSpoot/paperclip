...
import type { CreateIssueThreadInteraction, IssueDocumentSummary } from "@stapleai/shared";
 ...
 *
 * This implementation delegates to the core Staple domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@stapleai/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
