...
import { readStapleSkillSyncPreference } from "@stapleai/adapter-utils/server-utils";
import { claudeConfigDir, parseClaudeStreamJson } from "@stapleai/adapter-claude-local/server";
import { codexHomeDir, parseCodexJsonl } from "@stapleai/adapter-codex-local/server";
import { parseOpenCodeJsonl } from "@stapleai/adapter-opencode-local/server";
import {
...
import { resolveHomeAwarePath, resolveStapleInstanceRoot } from "../home-paths.js";
...
const FEEDBACK_SCHEMA_VERSION = "staple-feedback-envelope-v2";
const FEEDBACK_BUNDLE_VERSION = "staple-feedback-bundle-v2";
const FEEDBACK_PAYLOAD_VERSION = "staple-feedback-v1";
const FEEDBACK_DESTINATION = "staple_labs_feedback_v1";
...
function captureStatusFromFiles(files: FeedbackTraceBundleFile[]): FeedbackTraceBundleCaptureStatus {
  ...
  const hasAdapterFiles = files.some((file) =>
    file.source !== "staple_run" &&
    file.source !== "staple_run_events" &&
    file.source !== "staple_run_log",
  );
...
async function buildOpenCodeTraceFiles(input: {
  ...
  const opencodeRoot = resolveHomeAwarePath(
    process.env.STAPLE_OPENCODE_STORAGE_DIR ?? "~/.local/share/opencode",
  );
...
async function buildAgentContext(
  ...
  const desiredSkillRefs = uniqueNonEmpty(readStapleSkillSyncPreference(adapterConfig).desiredSkills).slice(0, MAX_SKILLS);
...
async function buildFeedbackTraceBundleFromRow(
  ...
  let stapleRun: Record<string, unknown> | null = null;
  ...
  if (!sourceRunId) {
    ...
    if (!run || run.companyId !== row.companyId) {
      ...
      stapleRun = sanitizeFeedbackValue(
        {
          id: run.id,
      ...
      files.push(makeBundleFile({
        path: "staple/run.json",
        contentType: "application/json",
        source: "staple_run",
        contents: `${JSON.stringify(stapleRun, null, 2)}\n`,
      }));

      const sanitizedEvents = sanitizeFeedbackValue(
        events,
        state,
        "bundle.stapleRun.events",
        MAX_TRACE_FILE_CHARS,
      );
      files.push(makeBundleFile({
        path: "staple/run-events.json",
        contentType: "application/json",
        source: "staple_run_events",
        contents: `${JSON.stringify(sanitizedEvents, null, 2)}\n`,
      }));

      if (logText) {
        files.push(makeBundleFile({
          path: "staple/run-log.ndjson",
          contentType: "application/x-ndjson",
          source: "staple_run_log",
          contents: `${sanitizeFeedbackText(logText, state, "bundle.stapleRun.log", MAX_TRACE_FILE_CHARS)}\n`,
        }));
      } else {
