...
import { ensureOpenCodeModelConfiguredAndAvailable } from "@stapleai/adapter-opencode-local/server";
...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".staple.yaml" || normalized === ".staple.yml") return "extension";
...
function readSkillKey(frontmatter: Record<string, unknown>) {
  ...
  const staple = isPlainRecord(metadata?.staple) ? metadata?.staple as Record<string, unknown> : null;
  return normalizeSkillKey(
    asString(frontmatter.key)
    ?? asString(frontmatter.skillKey)
    ?? asString(metadata?.skillKey)
    ?? asString(metadata?.canonicalKey)
    ?? asString(metadata?.stapleSkillKey)
    ?? asString(staple?.skillKey)
    ?? asString(staple?.key),
  );
}
...
function deriveManifestSkillKey(
  ...
  if (sourceKind === "staple_bundled") {
    return `stapleai/staple/${slug}`;
  }
...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (!frequency) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence without frequency; add .staple.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  if (interval < 1) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid interval; add .staple.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (startsAt && !zonedStartsAt) {
    errors.push(`Recurring task ${issue.slug} has an invalid legacy startsAt/timezone combination; add .staple.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid time; add .staple.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }

  if (issue.legacyRecurrence.until != null || issue.legacyRecurrence.count != null) {
    warnings.push(`Recurring task ${issue.slug} uses legacy recurrence end bounds; Staple will import the routine trigger without those limits.`);
  }
  ...
  if (frequency === "hourly") {
    ...
    if (Array.isArray(issue.legacyRecurrence.weekdays) || Array.isArray(issue.legacyRecurrence.monthDays) || Array.isArray(issue.legacyRecurrence.months)) {
      errors.push(`Recurring task ${issue.slug} uses unsupported legacy daily recurrence constraints; add .staple.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence with interval > 1; add .staple.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (cronWeekdays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence without weekdays; add .staple.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} * * ${normalizeCronList(cronWeekdays)}`;
  } else if (frequency === "monthly") {
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence with interval > 1; add .staple.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    if (Array.isArray(issue.legacyRecurrence.ordinalWeekdays) && issue.legacyRecurrence.ordinalWeekdays.length > 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy ordinal monthly recurrence; add .staple.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence without monthDays; add .staple.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence with interval > 1; add .staple.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    const months = Array.isArray(issue.legacyRecurrence.months)
      ? issue.legacyRecurrence.months
    ...
    if (months.length === 0 || monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence without month/monthDay anchors; add .staple.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} ${normalizeCronList(monthDays.map(String))} ${normalizeCronList(months.map(String))} *`;
  } else {
    errors.push(`Recurring task ${issue.slug} uses unsupported legacy recurrence frequency "${frequency}"; add .staple.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
...
function filterExportFiles(
  ...
  const extensionEntry = filtered[stapleExtensionPath];
  if (selectedFiles.has(stapleExtensionPath) && typeof extensionEntry === "string") {
    filtered[stapleExtensionPath] = filterPortableExtensionYaml(extensionEntry, selectedFiles);
  }

  return filtered;
}

function findStapleExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".staple.yaml"] === "string") return ".staple.yaml";
  if (typeof files[".staple.yml"] === "string") return ".staple.yml";
  return Object.keys(files).find((entry) => entry.endsWith("/.staple.yaml") || entry.endsWith("/.staple.yml")) ?? null;
}
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "staple_bundled") {
    const commit = await resolveBundledSkillsCommit();
    return {
      kind: "github-dir",
      repo: "stapleai/staple",
      path: `skills/${skill.slug}`,
      commit,
      trackingRef: "master",
      url: `https://github.com/stapleai/staple/tree/master/skills/${skill.slug}`,
    };
  }
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  ...
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "staple_bundled") return true;
...
async function withSkillSourceMetadata(skill: CompanySkill, markdown: string) {
  ...
  metadata.stapleSkillKey = skill.key;
  metadata.staple = {
    ...(isPlainRecord(metadata.staple) ? metadata.staple : {}),
    skillKey: skill.key,
    slug: skill.slug,
  };
  const frontmatter = {
...
function buildManifestFromPackageFiles(
  ...
  const stapleExtensionPath = findStapleExtensionPath(normalizedFiles);
  const stapleExtension = stapleExtensionPath
    ? parseYamlFile(readPortableTextFile(normalizedFiles, stapleExtensionPath) ?? "")
    : {};
  const stapleCompany = isPlainRecord(stapleExtension.company) ? stapleExtension.company : {};
  const stapleSidebar = normalizePortableSidebarOrder(stapleExtension.sidebar);
  const stapleAgents = isPlainRecord(stapleExtension.agents) ? stapleExtension.agents : {};
  const stapleProjects = isPlainRecord(stapleExtension.projects) ? stapleExtension.projects : {};
  const stapleTasks = isPlainRecord(stapleExtension.tasks) ? stapleExtension.tasks : {};
  const stapleRoutines = isPlainRecord(stapleExtension.routines) ? stapleExtension.routines : {};
  const companyName =
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      brandColor: asString(stapleCompany.brandColor),
      logoPath: asString(stapleCompany.logoPath) ?? asString(stapleCompany.logo),
      requireBoardApprovalForNewAgents:
        typeof stapleCompany.requireBoardApprovalForNewAgents === "boolean"
          ? stapleCompany.requireBoardApprovalForNewAgents
          : readCompanyApprovalDefault(companyFrontmatter),
      feedbackDataSharingEnabled:
        typeof stapleCompany.feedbackDataSharingEnabled === "boolean"
          ? stapleCompany.feedbackDataSharingEnabled
          : false,
      feedbackDataSharingConsentAt:
        typeof stapleCompany.feedbackDataSharingConsentAt === "string"
          ? stapleCompany.feedbackDataSharingConsentAt
          : null,
      feedbackDataSharingConsentByUserId:
        asString(stapleCompany.feedbackDataSharingConsentByUserId),
      feedbackDataSharingTermsVersion:
        asString(stapleCompany.feedbackDataSharingTermsVersion),
    },
    sidebar: stapleSidebar,
    agents: [],
    skills: [],
    projects: [],
    issues: [],
  ...
  for (const agentPath of agentPaths) {
    ...
    const extension = isPlainRecord(stapleAgents[slug]) ? stapleAgents[slug] : {};
  ...
  for (const projectPath of projectPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(stapleProjects[slug]) ? stapleProjects[slug] : {};
  ...
  for (const taskPath of taskPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(stapleTasks[slug]) ? stapleTasks[slug] : {};
    const routineExtension = normalizeRoutineExtension(stapleRoutines[slug]);
    const routineExtensionRaw = isPlainRecord(stapleRoutines[slug]) ? stapleRoutines[slug] : {};
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
  ...
  async function prepareImportedAgentAdapter(
    ...
    const nextAdapterConfig = writeStapleSkillSyncPreference({ ...adapterConfig }, desiredSkills);
  ...
  async function exportBundle(
    ...
    const stapleAgentsOut: Record<string, Record<string, unknown>> = {};
    const stapleProjectsOut: Record<string, Record<string, unknown>> = {};
    const stapleTasksOut: Record<string, Record<string, unknown>> = {};
    const unportableTaskWorkspaceRefs = new Map<string, { workspaceId: string; taskSlugs: string[] }>();
    const stapleRoutinesOut: Record<string, Record<string, unknown>> = {};
    ...
    if (include.agents) {
      for (const agent of agentRows) {
        ...
        const desiredSkills = readStapleSkillSyncPreference(
        ...
        stapleAgentsOut[slug] = isPlainRecord(extension) ? extension : {};
      }
    }
    ...
    for (const project of selectedProjectRows) {
      ...
      stapleProjectsOut[slug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const issue of selectedIssueRows) {
      ...
      stapleTasksOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const routine of selectedRoutineRows) {
      ...
      stapleRoutinesOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }

    const stapleExtensionPath = ".staple.yaml";
    const stapleAgents = Object.fromEntries(
      Object.entries(stapleAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const stapleProjects = Object.fromEntries(
      Object.entries(stapleProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const stapleTasks = Object.fromEntries(
      Object.entries(stapleTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const stapleRoutines = Object.fromEntries(
      Object.entries(stapleRoutinesOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    files[stapleExtensionPath] = buildYamlFile(
      {
        schema: "staple/v1",
        ...
        agents: Object.keys(stapleAgents).length > 0 ? stapleAgents : undefined,
        projects: Object.keys(stapleProjects).length > 0 ? stapleProjects : undefined,
        tasks: Object.keys(stapleTasks).length > 0 ? stapleTasks : undefined,
        routines: Object.keys(stapleRoutines).length > 0 ? stapleRoutines : undefined,
      },
      { preserveEmptyStrings: true },
    );

    let finalFiles = filterExportFiles(files, input.selectedFiles, stapleExtensionPath);
