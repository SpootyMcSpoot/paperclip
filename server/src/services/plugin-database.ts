...
export function pluginDatabaseService(db: Db) {
  ...
  async function ensureNamespace(pluginId: string, manifest: StaplePluginManifestV1) {
  ...
  return {
    ...
    async applyMigrations(pluginId: string, manifest: StaplePluginManifestV1, packageRoot: string) {
      if (!manifest.database) return null;
