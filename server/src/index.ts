...
export async function startServer(): Promise<StartedServer> {
  ...
  if (process.env.STAPLE_SECRETS_PROVIDER === undefined) {
    process.env.STAPLE_SECRETS_PROVIDER = config.secretsProvider;
  }
  if (process.env.STAPLE_SECRETS_STRICT_MODE === undefined) {
    process.env.STAPLE_SECRETS_STRICT_MODE = config.secretsStrictMode ? "true" : "false";
  }
  if (process.env.STAPLE_SECRETS_MASTER_KEY_FILE === undefined) {
    process.env.STAPLE_SECRETS_MASTER_KEY_FILE = config.secretsMasterKeyFilePath;
  }
  ...
  async function promptApplyMigrations(migrations: string[]): Promise<boolean> {
    if (process.env.STAPLE_MIGRATION_PROMPT === "never") return false;
    if (process.env.STAPLE_MIGRATION_AUTO_APPLY === "true") return true;
      ...
      if (!apply) {
        throw new Error(
          `${label} has pending migrations (${formatPendingMigrationSummary(state.pendingMigrations)}). ` +
            "Refusing to start against a stale schema. Run pnpm db:migrate or set STAPLE_MIGRATION_AUTO_APPLY=true.",
        );
      }
  ...
  const LOCAL_BOARD_USER_EMAIL = "local@staple.local";
  ...
  if (config.databaseUrl) {
    ...
    const verboseEmbeddedPostgresLogs = process.env.STAPLE_EMBEDDED_POSTGRES_VERBOSE === "true";
    ...
    if (runningPid) {
      ...
      const configuredAdminConnectionString = `postgres://staple:staple@127.0.0.1:${configuredPort}/postgres`;
      try {
        ...
        await ensurePostgresDatabase(configuredAdminConnectionString, "staple");
        logger.warn(
    ...
    const embeddedAdminConnectionString = `postgres://staple:staple@127.0.0.1:${port}/postgres`;
    const dbStatus = await ensurePostgresDatabase(embeddedAdminConnectionString, "staple");
    if (dbStatus === "created") {
      logger.info("Created embedded PostgreSQL database: staple");
    }
  
    const embeddedConnectionString = `postgres://staple:staple@127.0.0.1:${port}/staple`;
  ...
  if (config.deploymentMode === "authenticated") {
    ...
    const betterAuthSecret =
      process.env.BETTER_AUTH_SECRET?.trim() ?? process.env.STAPLE_AGENT_JWT_SECRET?.trim();
    if (!betterAuthSecret) {
      throw new Error(
        "authenticated mode requires BETTER_AUTH_SECRET (or STAPLE_AGENT_JWT_SECRET) to be set",
      );
    }
  ...
  const runtimeApiHost =
    runtimeListenHost === "0.0.0.0" || runtimeListenHost === "::"
      ? "localhost"
      : runtimeListenHost;
  process.env.STAPLE_LISTEN_HOST = runtimeListenHost;
  process.env.STAPLE_LISTEN_PORT = String(listenPort);
  process.env.STAPLE_API_URL = `http://${runtimeApiHost}:${listenPort}`;
  ...
  await new Promise<void>((resolveListen, rejectListen) => {
    ...
    server.listen(listenPort, config.host, () => {
      ...
      if (process.env.STAPLE_OPEN_ON_LISTEN === "true") {
  ...
  const shutdown = async (signal: "SIGINT" | "SIGTERM") => {
    logger.info({ signal }, "Shutting down Staple server");
  ...
  return {
    ...
    host: config.host,
    listenPort,
    apiUrl: process.env.STAPLE_API_URL ?? `http://${runtimeApiHost}:${listenPort}`,
...
if (isMainModule(import.meta.url)) {
  void startServer().catch((err) => {
    logger.error({ err }, "Staple server failed to start");
    process.exit(1);
  });
}
