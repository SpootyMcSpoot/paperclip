...
import type { StapleConfig } from "@stapleai/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by Staple worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.STAPLE_IN_WORKTREE !== "true") return null;

  const configPath = resolveStapleConfigPath(overrideConfigPath);
  const envPath = resolveStapleEnvPath(configPath);
  const persistedEnv = readEnvEntries(envPath);
  const worktreeRoot = path.resolve(path.dirname(configPath), "..");
  const worktreeName =
    nonEmpty(persistedEnv.STAPLE_WORKTREE_NAME) ??
    nonEmpty(env.STAPLE_WORKTREE_NAME) ??
    path.basename(worktreeRoot);
  const instanceId =
    nonEmpty(persistedEnv.STAPLE_INSTANCE_ID) ??
    nonEmpty(env.STAPLE_INSTANCE_ID) ??
    sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(persistedEnv.STAPLE_HOME) ??
      nonEmpty(env.STAPLE_HOME) ??
      nonEmpty(env.STAPLE_WORKTREES_DIR) ??
      "~/.staple-worktrees",
  );
...
function writeConfigFile(configPath: string, config: StapleConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".staple", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as StapleConfig;
...
): { config: StapleConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.STAPLE_HOME = context.homeDir;
  process.env.STAPLE_INSTANCE_ID = context.instanceId;
  process.env.STAPLE_CONFIG = context.configPath;
  process.env.STAPLE_CONTEXT = context.contextPath;
  process.env.STAPLE_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as StapleConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    STAPLE_HOME: context.homeDir,
    STAPLE_INSTANCE_ID: context.instanceId,
    STAPLE_CONFIG: context.configPath,
    STAPLE_CONTEXT: context.contextPath,
    STAPLE_IN_WORKTREE: "true",
    STAPLE_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: StapleConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as StapleConfig;
  } catch {
    return;
  }
