...
import { resolveStapleConfigPath, resolveStapleEnvPath } from "./paths.js";
import type { BindMode, DeploymentExposure, DeploymentMode } from "@stapleai/shared";
...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.STAPLE_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.STAPLE_AGENT_JWT_SECRET === "string" ? parsed.STAPLE_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  return {
    status: "warn",
    message: "missing (run `pnpm stapleai onboard`)",
  };
}
...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const envFilePath = resolveStapleEnvPath();
