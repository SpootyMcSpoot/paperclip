...
import type { CompanyPortabilityFileEntry } from "@stapleai/shared";
...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "staple",
        name: "staple",
        description: "Staple coordination skill",
        markdown: "---\nname: staple\ndescription: Staple coordination skill\n---\n\n# Staple\n",
        sourceType: "github",
        sourceLocator: "https://github.com/stapleai/staple/tree/master/skills/staple",
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/stapleai/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      hostname: "github.com",
      owner: "stapleai",
      repo: "companies",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/stapleai/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      hostname: "github.com",
      owner: "stapleai",
  ...
  it("exports referenced skills as stubs by default with sanitized Staple extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "Staple"');
    ...
    expect(asTextFile(exported.files["agents/claudecoder/AGENTS.md"])).toContain(`- "${stapleKey}"`);
    expect(asTextFile(exported.files["agents/cmo/AGENTS.md"])).not.toContain("skills:");
    expect(asTextFile(exported.files["skills/stapleai/staple/staple/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/stapleai/staple/staple/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/stapleai/staple/staple/references/api.md"]).toBeUndefined();
    ...
    const extension = asTextFile(exported.files[".staple.yaml"]);
    expect(extension).toContain('schema: "staple/v1"');
    ...
    expect(extension).not.toContain("stapleSkillSync");
  ...
  it("exports default sidebar order into the Staple extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files[".staple.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/stapleai/staple/staple/SKILL.md"])).toContain("# Staple");
    expect(asTextFile(exported.files["skills/stapleai/staple/staple/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/stapleai/staple/staple/references/api.md"])).toContain("# API");
  });
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/stapleai/staple/staple/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PAP/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/stapleai/staple/staple/SKILL.md"]).toBeDefined();
  });

  it("exports the company logo into images/ and references it from .staple.yaml", async () => {
    const storage = {
    ...
    expect(exported.files[".staple.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        id: "skill-staple",
        companyId: "company-1",
        key: "stapleai/staple/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/stapleai/staple/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
        metadata: {
          sourceKind: "staple_bundled",
          owner: "stapleai",
          repo: "staple",
          ref: "0123456789abcdef0123456789abcdef01234567",
          trackingRef: "master",
    ...
    expect(asTextFile(exported.files["skills/stapleai/staple/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/stapleai/staple/release-changelog/SKILL.md"])).toContain("stapleai/staple/release-changelog");
  });
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
        ...
        workspaces: [
          {
            ...
            name: "Main Repo",
            sourceType: "git_repo",
            cwd: "/Users/dotta/staple",
            repoUrl: "https://github.com/stapleai/staple.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
            setupCommand: "pnpm install",
            cleanupCommand: "rm -rf .staple-tmp",
            remoteProvider: null,
    ...
    expect(extension).toContain('repoUrl: "https://github.com/stapleai/staple.git"');
    expect(extension).toContain('defaultProjectWorkspaceKey: "main-repo"');
    expect(extension).toContain('projectWorkspaceKey: "main-repo"');
    expect(extension).not.toContain("/Users/dotta/staple");
    ...
    await portability.importBundle({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported Staple",
      },
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    const portability = companyPortabilityService({} as any);
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "staple-portability-git-"));
    execFileSync("git", ["init"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["checkout", "-b", "main"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["remote", "add", "origin", "https://github.com/stapleai/staple.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
    ...
    const extension = asTextFile(exported.files[".staple.yaml"]);
    expect(extension).toContain('repoUrl: "https://github.com/stapleai/staple.git"');
    expect(extension).toContain('projectWorkspaceKey: "staple"');
  ...
  it("reads env inputs back from .staple.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("reads project env inputs back from .staple.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("exports routines as recurring task packages with Staple routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("imports recurring task packages as routines instead of one-time issues", async () => {
    ...
    const preview = await portability.previewImport({
      source: { type: "inline", rootPath: "staple-demo", files },
      include: { company: true, agents: true, projects: true, issues: true, skills: false },
      target: { mode: "new_company", newCompanyName: "Imported Staple" },
      agents: "all",
      collisionStrategy: "rename",
    });
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported Staple"', "---", ""].join("\n"),
  ...
  it("imports a vendor-neutral package without .staple.yaml", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported Staple");
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".staple.yaml"] = `${exported.files[".staple.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
