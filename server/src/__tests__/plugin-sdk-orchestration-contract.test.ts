...
import type { Issue, StaplePluginManifestV1 } from "@stapleai/shared";
import { createTestHarness } from "../../../packages/plugins/sdk/src/testing.js";

function manifest(capabilities: StaplePluginManifestV1["capabilities"]): StaplePluginManifestV1 {
  return {
    id: "staple.test-orchestration",
    apiVersion: 1,
    version: "0.1.0",
...
describe("plugin SDK orchestration contract", () => {
  it("supports expanded issue create fields and relation helpers", async () => {
    ...
    expect(created.originKind).toBe("plugin:staple.test-orchestration");
  ...
  it("enforces plugin origin namespaces in the test harness", async () => {
    ...
    const created = await harness.ctx.issues.create({
      companyId,
      title: "Generated issue",
      originKind: "plugin:staple.test-orchestration:feature",
    });

    expect(created.originKind).toBe("plugin:staple.test-orchestration:feature");
    await expect(
    ...
    ).rejects.toThrow("Plugin may only use originKind values under plugin:staple.test-orchestration");
    await expect(
