...
import type { StaplePluginManifestV1 } from "@stapleai/shared";
import {
...
describeEmbeddedPostgres("plugin database namespaces", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("staple-plugin-db-");
    db = createDb(tempDb.connectionString);
  }, 20_000);

  afterEach(async () => {
    for (const pluginKey of ["staple.dbtest", "staple.escape"]) {
  ...
  async function createPluginPackage(manifest: StaplePluginManifestV1, migrationSql: string) {
    const packageRoot = await mkdtemp(path.join(os.tmpdir(), "staple-plugin-package-"));
  ...
  async function installPluginRecord(manifest: StaplePluginManifestV1) {
  ...
  function manifest(pluginKey = "staple.dbtest"): StaplePluginManifestV1 {
    return {
      id: pluginKey,
      apiVersion: 1,
      version: "1.0.0",
  ...
  it("records a failed migration when SQL escapes the plugin namespace", async () => {
    const pluginManifest = manifest("staple.escape");
    const packageRoot = await createPluginPackage(
      pluginManifest,
