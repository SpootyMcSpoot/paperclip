...
import { models as codexFallbackModels } from "@stapleai/adapter-codex-local";
import { models as cursorFallbackModels } from "@stapleai/adapter-cursor-local";
import { models as opencodeFallbackModels } from "@stapleai/adapter-opencode-local";
import { resetOpenCodeModelsCacheForTests } from "@stapleai/adapter-opencode-local/server";
...
describe("adapter model listing", () => {
  beforeEach(() => {
    ...
    delete process.env.STAPLE_OPENCODE_COMMAND;
  ...
  it("returns opencode fallback models including gpt-5.4", async () => {
    process.env.STAPLE_OPENCODE_COMMAND = "__staple_missing_opencode_command__";
