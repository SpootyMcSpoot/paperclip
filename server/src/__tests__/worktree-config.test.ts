...
describe("worktree config repair", () => {
  it("repairs legacy repo-local worktree config and env files into an isolated instance", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "staple-worktree-repair-"));
    const worktreeRoot = path.join(tempRoot, "PAP-884-ai-commits-component");
    const stapleDir = path.join(worktreeRoot, ".staple");
    const configPath = path.join(stapleDir, "config.json");
    const envPath = path.join(stapleDir, ".env");
    const sharedRoot = path.join(tempRoot, ".staple", "instances", "default");
    const isolatedHome = path.join(tempRoot, ".staple-worktrees");

    await fs.mkdir(stapleDir, { recursive: true });
    await fs.writeFile(configPath, JSON.stringify(buildLegacyConfig(sharedRoot), null, 2) + "\n", "utf8");
    await fs.writeFile(
      envPath,
      [
        "# Staple environment variables",
        "STAPLE_IN_WORKTREE=true",
        "STAPLE_WORKTREE_NAME=PAP-884-ai-commits-component",
        "STAPLE_AGENT_JWT_SECRET=shared-secret",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.env.STAPLE_WORKTREE_NAME = "PAP-884-ai-commits-component";
    process.env.STAPLE_WORKTREES_DIR = isolatedHome;
    ...
    expect(repairedEnv).toContain(`STAPLE_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).toContain('STAPLE_INSTANCE_ID="pap-884-ai-commits-component"');
    expect(repairedEnv).toContain(`STAPLE_CONFIG=${JSON.stringify(await fs.realpath(configPath))}`);
    expect(repairedEnv).toContain(`STAPLE_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`);
    expect(repairedEnv).toContain('STAPLE_AGENT_JWT_SECRET="shared-secret"');
    expect(process.env.STAPLE_HOME).toBe(isolatedHome);
    expect(process.env.STAPLE_INSTANCE_ID).toBe("pap-884-ai-commits-component");
  });

  it("avoids sibling worktree ports when repairing legacy configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "staple-worktree-repair-ports-"));
    ...
    await fs.writeFile(
      ...
      [
        "# Staple environment variables",
        "STAPLE_IN_WORKTREE=true",
        "STAPLE_WORKTREE_NAME=PAP-880-thumbs-capture-for-evals-feature",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.STAPLE_IN_WORKTREE = "true";
    process.env.STAPLE_WORKTREE_NAME = "PAP-880-thumbs-capture-for-evals-feature";
    process.env.STAPLE_WORKTREES_DIR = isolatedHome;
    delete process.env.STAPLE_HOME;
    delete process.env.STAPLE_INSTANCE_ID;
    delete process.env.STAPLE_CONFIG;
  ...
  it("does not persist transient runtime home overrides over repo-local worktree env", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "staple-worktree-runtime-override-"));
    const isolatedHome = path.join(tempRoot, ".staple-worktrees");
    ...
    await fs.writeFile(
      ...
      [
        "# Staple environment variables",
        `STAPLE_HOME=${JSON.stringify(isolatedHome)}`,
        `STAPLE_INSTANCE_ID=${JSON.stringify(instanceId)}`,
        `STAPLE_CONFIG=${JSON.stringify(configPath)}`,
        `STAPLE_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`,
        'STAPLE_IN_WORKTREE="true"',
        'STAPLE_WORKTREE_NAME="PAP-989-multi-user-implementation-using-plan-from-pap-958"',
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.STAPLE_IN_WORKTREE = "true";
    process.env.STAPLE_WORKTREE_NAME = "PAP-989-multi-user-implementation-using-plan-from-pap-958";
    process.env.STAPLE_HOME = transientHome;
    ...
    expect(repairedEnv).toContain(`STAPLE_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).not.toContain(`STAPLE_HOME=${JSON.stringify(transientHome)}`);
    expect(process.env.STAPLE_HOME).toBe(isolatedHome);
  });

  it("rebalances duplicate ports for already isolated worktree configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "staple-worktree-rebalance-"));
    const isolatedHome = path.join(tempRoot, ".staple-worktrees");
    const repoWorktreesRoot = path.join(tempRoot, "repo", ".staple", "worktrees");
    ...
    const stapleDir = path.join(currentWorktreeRoot, ".staple");
    const configPath = path.join(stapleDir, "config.json");
    const envPath = path.join(stapleDir, ".env");
    const currentInstanceRoot = path.join(isolatedHome, "instances", "pap-884-ai-commits-component");
    const siblingConfigPath = path.join(siblingWorktreeRoot, ".staple", "config.json");
  ...
  it("persists runtime-selected worktree ports back into config", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "staple-worktree-ports-"));
    ...
    process.chdir(worktreeRoot);
    process.env.STAPLE_IN_WORKTREE = "true";
    process.env.STAPLE_WORKTREE_NAME = "PAP-878-create-a-mine-tab-in-inbox";
    process.env.STAPLE_HOME = isolatedHome;
    process.env.STAPLE_INSTANCE_ID = "pap-878-create-a-mine-tab-in-inbox";
    process.env.STAPLE_CONFIG = configPath;
