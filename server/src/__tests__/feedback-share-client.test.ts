...
describe("feedback trace share client", () => {
  ...
  it("defaults to telemetry.staple.ing when no backend url is configured", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      "https://telemetry.staple.ing/feedback-traces",
  ...
  it("wraps the feedback trace payload as gzip+base64 json before upload", async () => {
    const client = createFeedbackTraceShareClientFromConfig({
      feedbackExportBackendUrl: "https://telemetry.staple.ing",
    ...
    expect(call?.[0]).toBe("https://telemetry.staple.ing/feedback-traces");
