...
const ORIGINAL_STAPLE_RUNTIME_API_URL = process.env.STAPLE_RUNTIME_API_URL;
...
afterEach(() => {
  if (ORIGINAL_STAPLE_RUNTIME_API_URL === undefined) delete process.env.STAPLE_RUNTIME_API_URL;
  else process.env.STAPLE_RUNTIME_API_URL = ORIGINAL_STAPLE_RUNTIME_API_URL;
...
describe("buildStapleEnv", () => {
  it("prefers an explicit STAPLE_RUNTIME_API_URL", () => {
    process.env.STAPLE_RUNTIME_API_URL = "http://100.104.161.29:3102";
    ...
    expect(env.STAPLE_API_URL).toBe("http://100.104.161.29:3102");
  });

  it("falls back to STAPLE_API_URL when no runtime URL is configured", () => {
    delete process.env.STAPLE_RUNTIME_API_URL;
    process.env.STAPLE_API_URL = "http://localhost:4100";
    process.env.STAPLE_LISTEN_HOST = "127.0.0.1";
    process.env.STAPLE_LISTEN_PORT = "3101";
