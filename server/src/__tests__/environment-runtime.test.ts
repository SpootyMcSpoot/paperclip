...
describeEmbeddedPostgres("environmentRuntimeService", () => {
  ...
  it("rejects truly unsupported drivers before acquiring a lease", async () => {
    const { companyId, environment, runId } = await seedEnvironment({
      ...
      config: {
        ...
        port: 22,
        username: "ssh-user",
        remoteWorkspacePath: "/srv/staple/workspace",
        privateKey: null,
        knownHosts: null,
  ...
  it("acquires and releases an SSH run lease through the runtime seam", async () => {
    ...
    const fixtureRoot = await mkdtemp(path.join(os.tmpdir(), "staple-environment-runtime-ssh-"));
    ...
    const previousCandidates = process.env.STAPLE_RUNTIME_API_CANDIDATES_JSON;
    process.env.STAPLE_RUNTIME_API_CANDIDATES_JSON = JSON.stringify([runtimeApiUrl]);
    const { companyId, environment, runId } = await seedEnvironment({
      driver: "ssh",
      name: "Fixture SSH",
      config: sshConfig,
    ...
    try {
      ...
      if (previousCandidates === undefined) {
        delete process.env.STAPLE_RUNTIME_API_CANDIDATES_JSON;
      } else {
        process.env.STAPLE_RUNTIME_API_CANDIDATES_JSON = previousCandidates;
      }
      await new Promise<void>((resolve) => healthServer.close(() => resolve()));
    }
  });
  ...
  it("uses plugin-backed sandbox config for execute and release", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "staple.fake-plugin-sandbox-provider",
      packageName: "@stapleai/plugin-fake-sandbox",
      version: "1.0.0",
      apiVersion: 1,
      categories: ["automation"],
      manifestJson: {
        id: "staple.fake-plugin-sandbox-provider",
        apiVersion: 1,
        version: "1.0.0",
  ...
  it("delegates plugin environment leases through the plugin worker manager", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "acme.environments",
      packageName: "@acme/staple-environments",
      version: "1.0.0",
      apiVersion: 1,
      categories: ["automation"],
      manifestJson: {
