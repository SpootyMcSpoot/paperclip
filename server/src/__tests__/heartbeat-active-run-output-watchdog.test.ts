...
vi.mock("@stapleai/shared/telemetry", async () => {
  const actual = await vi.importActual<typeof import("@stapleai/shared/telemetry")>(
    "@stapleai/shared/telemetry",
  );
  return {
    ...actual,
...
describeEmbeddedPostgres("active-run output watchdog", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("staple-active-run-output-watchdog-");
