...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Staple OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("/api/invites/token-123/skills/staple");
    expect(text).toContain("Suggested Staple base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("stapleApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.stapleApiUrl");
    expect(text).toContain("~/.openclaw/workspace/staple-claimed-api-key.json");
    expect(text).toContain("STAPLE_API_KEY");
