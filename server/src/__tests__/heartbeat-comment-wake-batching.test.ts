...
describe("heartbeat comment wake batching", () => {
  ...
  beforeAll(async () => {
    const started = await startEmbeddedPostgresTestDatabase("staple-heartbeat-comment-wake-");
  ...
  it("defers approval-approved wakes for a running issue so the assignee resumes after the run", async () => {
    ...
    expect((deferred?.payload as Record<string, unknown>)._stapleWakeContext).toMatchObject({
      issueId,
      taskId: issueId,
      approvalId: "approval-1",
      approvalStatus: "approved",
  ...
  it("batches deferred comment wakes and forwards the ordered batch to the next run", async () => {
    ...
    try {
      ...
      const deferredContext = (deferredWake?.payload as Record<string, unknown> | null)?._stapleWakeContext as
      ...
      expect(secondPayload.staple).toMatchObject({
        wake: {
  ...
  it("promotes deferred comment wakes with their comments after the active run is cancelled", async () => {
    ...
    try {
      ...
      expect(promotedPayload.staple).toMatchObject({
        wake: {
  ...
  it("queues exactly one follow-up run when an issue-bound run exits without a comment", async () => {
    ...
    try {
      ...
      expect(firstPayload.staple).toMatchObject({
        wake: {
          reason: "issue_assigned",
          issue: {
            id: issueId,
      ...
      expect(String(firstPayload.message ?? "")).toContain("## Staple Wake Payload");
