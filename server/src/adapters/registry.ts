...
import { agentConfigurationDoc as claudeAgentConfigurationDoc, models as claudeModels } from "@stapleai/adapter-claude-local";
import {
...
import { agentConfigurationDoc as codexAgentConfigurationDoc, models as codexModels } from "@stapleai/adapter-codex-local";
import {
...
import { agentConfigurationDoc as cursorAgentConfigurationDoc, models as cursorModels } from "@stapleai/adapter-cursor-local";
import {
...
import { agentConfigurationDoc as geminiAgentConfigurationDoc, models as geminiModels } from "@stapleai/adapter-gemini-local";
import {
...
} from "@stapleai/adapter-opencode-local";
import {
...
} from "@stapleai/adapter-pi-local";
import {
...
const hermesLocalAdapter: ServerAdapterModule = {
  ...
  execute: async (ctx) => {
    ...
    const explicitApiKey =
      typeof existingEnv.STAPLE_API_KEY === "string" && existingEnv.STAPLE_API_KEY.trim().length > 0;
    ...
    const authGuardPrompt = [
      "Staple API safety rule:",
      "Use Authorization: Bearer $STAPLE_API_KEY on every Staple API request.",
      "Use X-Staple-Run-Id: $STAPLE_RUN_ID on every Staple API request that writes or mutates data, including comments and issue updates.",
      "Never use a board, browser, or local-board session for Staple API writes.",
    ].join("\n");
    ...
    const patchedConfig: Record<string, unknown> = {
      ...
      env: {
        ...existingEnv,
        ...(!explicitApiKey ? { STAPLE_API_KEY: normalizedCtx.authToken } : {}),
        STAPLE_RUN_ID: normalizedCtx.runId,
      },
    };
...
const externalAdaptersReady: Promise<void> = (async () => {
  try {
    ...
    for (const externalAdapter of externalAdapters) {
      ...
      if (overriding) {
        console.log(
          `[staple] External adapter "${externalAdapter.type}" overrides built-in adapter`,
        );
    ...
    console.error("[staple] Failed to load external adapters:", err);
  }
})();
...
export function setOverridePaused(type: string, paused: boolean): boolean {
  ...
  if (paused && !wasPaused) {
    ...
    console.log(`[staple] Override paused for "${type}" — builtin adapter restored`);
    return true;
  }
  if (!paused && wasPaused) {
    pausedOverrides.delete(type);
    console.log(`[staple] Override resumed for "${type}" — external adapter active`);
    return true;
  }
  return false;
}
