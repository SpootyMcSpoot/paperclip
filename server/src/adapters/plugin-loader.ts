...
function extractUiParserSource(
  ...
  const contractVersion = pkg.staple?.adapterUiParser;
  if (contractVersion) {
    ...
    logger.info(
      { packageName },
      "Adapter has ./ui-parser export but no staple.adapterUiParser version — loading anyway (future versions may require it)",
    );
  }
