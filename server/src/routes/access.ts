...
import type { DeploymentExposure, DeploymentMode, HumanCompanyMembershipRole, PermissionKey } from "@stapleai/shared";
import {
  forbidden,
  conflict,
  notFound,
...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "staple" &&
    normalized !== "staple-create-agent" &&
    normalized !== "staple-create-plugin" &&
...
/** Resolve the Staple repo skills directory (built-in / managed skills). */
function resolveStapleSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const stapleSkillsDir = resolveStapleSkillsDir();

  // Build set of Staple-managed skill names
  const stapleSkillNames = new Set<string>();
  if (stapleSkillsDir) {
    try {
      for (const entry of fs.readdirSync(stapleSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) stapleSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isStapleManaged: stapleSkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.stapleApiUrl)) {
    const legacyStapleApiUrl = nonEmptyTrimmedString(input.stapleApiUrl);
    if (legacyStapleApiUrl) merged.stapleApiUrl = legacyStapleApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    stapleApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.stapleApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawStapleApiUrl =
    typeof defaults.stapleApiUrl === "string"
      ? defaults.stapleApiUrl.trim()
      : "";
  if (rawStapleApiUrl) {
    try {
      const parsedStapleApiUrl = new URL(rawStapleApiUrl);
      if (
        parsedStapleApiUrl.protocol !== "http:" &&
        parsedStapleApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_staple_api_url_protocol",
          level: "warn",
          message: `stapleApiUrl must use http:// or https:// (got ${parsedStapleApiUrl.protocol}).`
        });
      } else {
        normalized.stapleApiUrl = parsedStapleApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_staple_api_url_configured",
          level: "info",
          message: `stapleApiUrl set to ${parsedStapleApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_staple_api_url_invalid",
        level: "warn",
        message: `Invalid stapleApiUrl: ${rawStapleApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Staple host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Staple is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm stapleai allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = `/api/invites/${token}/skills/staple`;
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/staple-claimed-api-key.json and load STAPLE_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      inviteMessage: extractInviteMessage(invite),
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: stapleApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Staple hostname is reachable and allowed via `pnpm stapleai allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Staple API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/staple/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # Staple OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          stapleApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, Staple generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "stapleApiUrl": "https://staple-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in Staple before key claim is allowed.
    ...
    - ~/.openclaw/workspace/staple-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/staple-claimed-api-key.json

    And set the STAPLE_API_KEY and STAPLE_API_URL in your environment variables as specified here:
    ...
    Then set STAPLE_API_KEY and STAPLE_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install Staple skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your STAPLE_API_URL to the top of your skill and note the path to your STAPLE_API_URL
    ...
    ${
      ...
      "Ensure Staple is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Staple base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.stapleApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm stapleai allowed-hostname <host>
      - then restart Staple and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@staple.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (req, res) => {
    ...
    res.json({
      skills: [
        { name: "staple", path: "/api/skills/staple" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "staple-create-agent",
          path: "/api/skills/staple-create-agent"
        }
      ]
    });
  });
  ...
  router.get("/invites/:token/skills/index", async (req, res) => {
    ...
    res.json({
      skills: [
        {
          name: "staple",
          path: `/api/invites/${token}/skills/staple`,
        },
      ],
    });
  });
  ...
  router.get("/invites/:token/skills/:skillName", async (req, res) => {
    ...
    const skillName = (req.params.skillName as string).trim().toLowerCase();
    if (skillName !== "staple") throw notFound("Skill not found");
    const markdown = readSkillMarkdown(skillName);
    if (!markdown) throw notFound("Skill not found");
    res.type("text/markdown").send(markdown);
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              stapleApiUrl: req.body.stapleApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.stapleApiUrl &&
          !persistedDefaults.stapleApiUrl
        ) {
          missingPersistedFields.push("stapleApiUrl");
        }
