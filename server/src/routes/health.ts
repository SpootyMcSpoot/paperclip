...
import { heartbeatRuns, instanceUserRoles, invites } from "@stapleai/db";
...
function hasDevServerStatusToken(providedToken: string | undefined) {
  const expectedToken = process.env.STAPLE_DEV_SERVER_STATUS_TOKEN?.trim();
...
export function healthRoutes(
  ...
  router.get("/", async (req, res) => {
    ...
    const exposeDevServerDetails =
      exposeFullDetails || hasDevServerStatusToken(req.get("x-staple-dev-server-status-token"));
