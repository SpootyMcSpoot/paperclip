 ...
 * toggle server-side adapter code for the whole Staple instance.
 *
...
interface AdapterInstallRequest {
  /** npm package name (e.g., "droid-staple-adapter") or local path */
  packageName: string;
...
export function adapterRoutes() {
  ...
  router.post("/adapters/install", async (req, res) => {
    ...
    // e.g. "@henkey/ HERMES_STAPLE_ADAPTER @0.3.0" → packageName + version
  ...
  // without modifying Staple's source code.
  //
