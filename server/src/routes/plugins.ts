...
import type { ToolRunContext } from "@stapleai/plugin-sdk";
import { JsonRpcCallError, PLUGIN_RPC_ERROR_CODES } from "@stapleai/plugin-sdk";
import {
  assertAuthenticated,
  assertBoard,
...
type PluginUiSlotDeclaration = NonNullable<NonNullable<StaplePluginManifestV1["ui"]>["slots"]>[number];
/** Launcher declaration extracted from plugin manifest */
type PluginLauncherDeclaration = NonNullable<StaplePluginManifestV1["launchers"]>[number];
...
interface PluginInstallRequest {
  /** npm package name (e.g., @staple/plugin-linear) or local path */
  packageName: string;
...
const BUNDLED_PLUGIN_EXAMPLES: AvailablePluginExample[] = [
  {
    packageName: "@stapleai/plugin-hello-world-example",
    pluginKey: "staple.hello-world-example",
  ...
  {
    packageName: "@stapleai/plugin-file-browser-example",
    pluginKey: "staple-file-browser-example",
  ...
  {
    packageName: "@stapleai/plugin-kitchen-sink-example",
    pluginKey: "staple-kitchen-sink-example",
    displayName: "Kitchen Sink (Example)",
    description: "Reference plugin that demonstrates the current Staple plugin API surface, bridge flows, UI extension surfaces, jobs, webhooks, tools, streams, and trusted local workspace/process demos.",
    localPath: "packages/plugins/examples/plugin-kitchen-sink-example",
    tag: "example",
  },
  {
    packageName: "@stapleai/plugin-orchestration-smoke-example",
    pluginKey: "stapleai.plugin-orchestration-smoke-example",
