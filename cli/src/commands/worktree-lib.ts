...
export const DEFAULT_WORKTREE_HOME = "~/.staple-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".staple");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: StapleConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "staple",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    STAPLE_HOME: paths.homeDir,
    STAPLE_INSTANCE_ID: paths.instanceId,
    STAPLE_CONFIG: paths.configPath,
    STAPLE_CONTEXT: paths.contextPath,
    STAPLE_IN_WORKTREE: "true",
    ...(branding?.name ? { STAPLE_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { STAPLE_WORKTREE_COLOR: branding.color } : {}),
  };
}
