...
import { ApiRequestError, StapleApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to Staple config file")
    .option("-d, --data-dir <path>", "Staple data directory root (isolates state from ~/.staple)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Staple API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.STAPLE_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const apiKey =
    options.apiKey?.trim() ||
    process.env.STAPLE_API_KEY?.trim() ||
    readKeyFromProfileEnv(profile);

  const companyId =
    options.companyId?.trim() ||
    process.env.STAPLE_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set STAPLE_COMPANY_ID, or set context profile companyId via `stapleai context set`.",
    );
  }

  const api = new StapleApiClient({ apiBase, apiKey });
  return {
    api,
    companyId,
    profileName,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.STAPLE_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.STAPLE_SERVER_PORT || "");
