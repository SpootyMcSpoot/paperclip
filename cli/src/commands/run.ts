...
import { loadStapleEnvFile } from "../config/env.js";
...
export async function runCommand(opts: RunOptions): Promise<void> {
  const instanceId = resolveStapleInstanceId(opts.instance);
  process.env.STAPLE_INSTANCE_ID = instanceId;

  const homeDir = resolveStapleHomeDir();
  ...
  loadStapleEnvFile(configPath);

  p.intro(pc.bgCyan(pc.black(" stapleai run ")));
  ...
  if (!configExists(configPath)) {
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
      ...
      p.log.message(`Run ${pc.cyan("stapleai onboard")} once, then retry ${pc.cyan("stapleai run")}.`);
      process.exit(1);
    }
  ...
  p.log.step("Starting Staple server...");
...
function maybeEnableUiDevMiddleware(entrypoint: string): void {
  if (process.env.STAPLE_UI_DEV_MIDDLEWARE !== undefined) return;
  const normalized = entrypoint.replaceAll("\\", "/");
  if (normalized.endsWith("/server/src/index.ts") || normalized.endsWith("@stapleai/server/src/index.ts")) {
    process.env.STAPLE_UI_DEV_MIDDLEWARE = "true";
  }
}
...
function ensureDevWorkspaceBuildDeps(projectRoot: string): void {
  ...
  if (result.error) {
    throw new Error(
      `Failed to prepare workspace build artifacts before starting the Staple dev server.\n${formatError(result.error)}`,
    );
  }
...
async function importServerEntry(): Promise<StartedServer> {
  ...
  // Production mode: import the published @stapleai/server package
  try {
    const mod = await import("@stapleai/server");
    return await startServerFromModule(mod, "@stapleai/server");
  } catch (err) {
    const missingSpecifier = getMissingModuleSpecifier(err);
    const missingServerEntrypoint = !missingSpecifier || missingSpecifier === "@stapleai/server";
    if (isModuleNotFoundError(err) && missingServerEntrypoint) {
      throw new Error(
        `Could not locate a Staple server entrypoint.\n` +
          `Tried: ${devEntry}, @stapleai/server\n` +
          `${formatError(err)}`,
      );
    }
    throw new Error(
      `Staple server failed to start.\n` +
        `${formatError(err)}`,
    );
  }
}

function shouldGenerateBootstrapInviteAfterStart(config: StapleConfig): boolean {
...
async function startServerFromModule(mod: unknown, label: string): Promise<StartedServer> {
  ...
  if (typeof startServer !== "function") {
    throw new Error(`Staple server entrypoint did not export startServer(): ${label}`);
  }
