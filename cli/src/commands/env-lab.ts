...
import { resolveStapleInstanceId, resolveStapleInstanceRoot } from "../config/home.js";
...
export function resolveEnvLabSshStatePath(instanceId?: string): string {
  ...
  return path.resolve(
    resolveStapleInstanceRoot(resolvedInstanceId),
    "env-lab",
    "ssh-fixture",
    "state.json",
  );
...
export async function envLabDoctorCommand(opts: { instance?: string; json?: boolean }) {
  ...
  p.log.message(`Cleanup: ${pc.dim("pnpm stapleai env-lab down")}`);
}
...
export function registerEnvLabCommands(program: Command) {
  ...
  envLab
    ...
    .option("-i, --instance <id>", "Staple instance id (default: current/default)")
