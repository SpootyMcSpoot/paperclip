...
describe("cli telemetry", () => {
  ...
  it("respects telemetry.enabled=false from the config file", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "staple-cli-telemetry-"));
    const configPath = makeConfigPath(root, false);
    process.env.STAPLE_HOME = path.join(root, "home");
    process.env.STAPLE_INSTANCE_ID = "telemetry-test";
