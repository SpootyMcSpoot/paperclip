...
describe("StapleApiClient", () => {
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new StapleApiClient({
    ...
    expect(headers["x-staple-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new StapleApiClient({ apiBase: "http://localhost:3100" });
