...
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "@stapleai/adapter-utils";

export interface StatefulStdoutParser {
  parseLine: (line: string, ts: string) => import("@stapleai/adapter-utils").TranscriptEntry[];
  reset: () => void;
}
...
export interface TranscriptParserSource {
  parseStdoutLine: (line: string, ts: string) => import("@stapleai/adapter-utils").TranscriptEntry[];
