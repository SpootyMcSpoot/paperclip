...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="Staple API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "stapleApiUrl",
                  String(config.stapleApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "stapleApiUrl", v || undefined)}
              immediate
              className={inputClass}
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "staple"))}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. Staple persists a device key during onboarding so pairing approvals
