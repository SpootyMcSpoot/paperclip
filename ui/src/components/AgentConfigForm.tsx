...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  const showSessionCompactionCard = Boolean(
    sessionCompaction.adapterSessionManagement
  );
            ...
            {isLocal && (
              <>
                ...
                <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                  Prompt template is replayed on every heartbeat. Keep it
                  compact and dynamic to avoid recurring token cost and cache
                  churn.
                </div>
              </>
            )}
        ...
        >
          ...
          {isLocal && isCreate && (
            <>
              ...
              <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                Prompt template is replayed on every heartbeat. Prefer small
                task framing and variables like <code>{"{{ context.* }}"}</code>{" "}
                or <code>{"{{ run.* }}"}</code>; avoid repeating stable
                instructions here.
              </div>
            </>
          )}
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          >
            ...
            <div className="rounded-md border border-sky-500/25 bg-sky-500/10 px-3 py-2 text-xs text-sky-100">
              Bootstrap prompt is only sent for fresh sessions. Put stable
              setup, habits, and longer reusable guidance here. Frequent changes
              reduce the value of session reuse because new sessions must replay
              it.
            </div>
            {adapterType === "claude_local" && (
...
function SessionCompactionPolicyCard({
  ...
  const sourceLabel =
    source === "agent_override" ? "Agent override" : "Adapter default";
  ...
  return (
    <div className="rounded-md border border-sky-500/25 bg-sky-500/10 px-3 py-3 space-y-2">
      <div className="flex items-center justify-between gap-3">
        <div className="text-xs font-medium text-sky-50">
          Session compaction
        </div>
      ...
      <p className="text-[11px] text-sky-100/75">
        A large cumulative raw token total does not mean the full session is
        resent on every heartbeat.
