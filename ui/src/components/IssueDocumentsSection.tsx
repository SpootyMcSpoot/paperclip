...
import { isSystemIssueDocumentKey } from "@stapleai/shared";
...
const getFoldedDocumentsStorageKey = (issueId: string) => `staple:issue-document-folds:${issueId}`;
...
export function IssueDocumentsSection({
  ...
  const documentBodyContentClassName = "staple-edit-in-place-content min-h-[220px] text-[15px] leading-7";
        ...
        >
          ...
          {!isPlanKey(draft.key) && (
            <Input
              ...
              aria-label="Document title (optional)"
            />
          )}
          <MarkdownEditor
            value={draft.body}
