...
function MarkdownIssueLink({
  ...
  return (
    <Link
      ...
      className="staple-markdown-issue-ref"
      title={title}
...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`staple-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="staple-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("staple-mermaid-status", error && "staple-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="staple-mermaid-source">
...
export function MarkdownBody({
      ...
      if (parsed) {
        ...
        return (
          <a
            ...
            className={cn(
              "staple-mention-chip",
              `staple-mention-chip--${parsed.kind}`,
              parsed.kind === "project" && "staple-project-mention-chip",
            )}
  ...
  return (
    <div
      className={cn(
        "staple-markdown prose prose-sm min-w-0 max-w-full break-words overflow-hidden",
