...
/**
 * Filter .staple.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterStapleYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://staple.ing) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm stapleai company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [Staple](https://staple.ing) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [Staple](https://staple.ing) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .staple.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .staple.yaml
    const yamlPath = exportData.stapleExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterStapleYaml(exportData.files[yamlPath], checkedFiles);
    }
