...
name: release
description: >
  Coordinate a full Staple release across engineering verification, npm,
...
Run the full Staple release as a maintainer workflow, not just an npm publish.
...
7. If running through Staple, you have issue context for status updates and follow-up task creation.
...
Staple now uses this release model:
...
```bash
npm view stapleai@canary version
```
...
- any release summary in Staple issue context
